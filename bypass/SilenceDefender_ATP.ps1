...
.DESCRIPTION
   ...
   start showing up in Microsoft Defender security center. This cmdlet allows the max of 3 diferent port
   inputs to be blocked on -LocalPort \ -RemotePort parameter declarations (split the ports with commas).
...
.EXAMPLE
   PS C:\> .\SilenceDefender_ATP.ps1 -Action Create -DisplayName "Block firefox" -LocalPort "Any" -RemotePort "443" -Program "$Env:ProgramFiles\Mozilla Firefox\firefox.exe" -Direction Outbound
   Create a new firewall rule that Blocks firefox.exe from connecting with remoteport 443 and localport 'Any' (Outbound).

.EXAMPLE
   PS C:\> .\SilenceDefender_ATP.ps1 -Action Create -DisplayName "Block-PS" -LocalPort "80,443" -RemotePort "any" -Program "$PSHOME\powershell.exe" -Direction Inbound
   Create a new firewall rule that Blocks powershell.exe from connecting with localport 80,443 and remoteport 'Any' (Inbound).
...
If($Action -ieq "Create")
{
   ...
   Write-Host "`n* Create program\application 'Block' firewall rule!" -ForegroundColor Blue
