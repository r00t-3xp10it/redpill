...
.EXAMPLE
   PS C:\> .\SilenceDefender_ATP.ps1 -Action Create
   Create a program\appl new 'Block' firewall rule manually.
...
If($Action -ieq "Create")
{
   ...
   If(-not($IsClientAdmin))
   {
      ...
      Write-Host "   Error: Administrator privileges required to create rules .." -ForegroundColor Red -BackgroundColor Black
      Write-Host "`n";Exit #Exit SilenceDefender_ATP
   }

   Write-Host "`n* Create a program\application 'Block' firewall rule!" -ForegroundColor Blue
   Write-Host "  => Remark : Program input only accepts cmd environement variables." -ForegroundColor Yellow
   Write-Host "  => Example: %programfiles%\Mozilla\Firefox\firefox.exe`n" -ForegroundColor Yellow

   Write-Host "New firewall rule DisplayName : " -NoNewline;
   $DisplayName = Read-Host;
   Write-Host "Program to be 'blocked' path  : " -NoNewline;
   $Program = Read-Host;
   Write-Host "The program Port to be blocked: " -NoNewline;
   $RemotePort = Read-Host;
   Write-Host "Direction (Outbound|Inbound)  : " -NoNewline;
   $Direction = Read-Host;

   #Create new firewall 'Block' rule
...
If($Action -ieq "Silence")
{
   ...
   If(-not($IsClientAdmin))
   {
      ...
      Write-Host "   Error: Administrator privileges required to add rules .." -ForegroundColor Red -BackgroundColor Black
      Write-Host "`n";Exit #Exit SilenceDefender_ATP
   }
...
If($Action -ieq "Delete")
{
   ...
   If(-not($IsClientAdmin))
   {
      ...
      Write-Host "   Error: Administrator privileges required to delete rules .." -ForegroundColor Red -BackgroundColor Black
      Write-Host "`n";Exit #Exit SilenceDefender_ATP
   }
