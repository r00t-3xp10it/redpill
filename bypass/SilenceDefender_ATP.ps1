...
If($Action -ieq "Create")
{
   ...
   If(-not(Test-Path -Path "$Program" -EA SilentlyContinue))
   {
      Write-Host "   => Error: not found '$Program'" -ForegroundColor Red -BackgroundColor Black
      $Program = "%windir%\system32\WindowsPowerShell\v1.0\powershell.exe";Start-Sleep -Milliseconds 500
      Write-Host "   => Fix  : defaulting to '$Program'" -ForegroundColor blue
   }
