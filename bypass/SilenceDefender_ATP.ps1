...
.EXAMPLE
   PS C:\> .\SilenceDefender_ATP.ps1 -Action Create -DisplayName "Block firefox" -RemotePort "443" -Program "%ProgramFiles%\Mozilla Firefox\firefox.exe"
...
If($Action -ieq "Create")
{
   ...
   Write-Host "`n* Create a program\application 'Block' firewall rule!" -ForegroundColor Blue
   If(-not($RemotePort) -or $RemotePort -ieq $null){$RemotePort = "443"}
   ...
   If(-not($Program) -or $Program -ieq $null){$Program = "%ProgramFiles%\Mozilla Firefox\firefox.exe"}
