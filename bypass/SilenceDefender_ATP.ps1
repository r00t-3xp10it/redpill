...
.NOTES
   It seems that all events are cached locally, so once the firewall rules are removed,
   the events and alerts will start showing up in Microsoft Defender Security Center. so
   its adviced to run '.\redpill.ps1 -GetLogs deleteall' to delete problematic logfiles
   before activating microsoft defender firewall rules again ..
...
If($Action -ieq "Silence")
{
   ...
   {
      Write-Host "`n   Error: Administrator privileges required .." -ForegroundColor Red -BackgroundColor Black
      Write-Host "";Exit #Exit SilenceDefender_ATP
   }
...
If($QueryRules)
{
   ...
   #Create logfile for redpill to be abble to read\delete it..
   #echo $QueryRules | Format-Table -AutoSize | Out-File -FilePath "$Env:TMP\SDATP.redpill" -Force
