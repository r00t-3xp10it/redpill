...
If($Action -ieq "Create")
{
   ...
   try{#Create new firewall 'Block' rule
      New-NetFirewallRule -DisplayName "$DisplayName" -Name "$DisplayName" -Direction $Direction -Program "$Program" -LocalPort "$LocalPort" -RemotePort "$RemotePort" -Protocol TCP -Action Block|Out-Null
