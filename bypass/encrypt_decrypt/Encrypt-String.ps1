...
# NOTE: The random function generates byte keys from '240' to '255' values.
[Byte[]]$SecretKey = 117,9,103,192,133,20,53,149,83,95,108,34,82,224,226,220,56,68,133,120,139,238,176,239,171,54,231,205,83,57,51,$RoolTheDiceAgain
$DisplaySecret = "117,9,103,192,133,20,53,149,83,95,108,34,82,224,226,220,56,68,133,120,139,238,176,239,171,54,231,205,83,57,51,$RoolTheDiceAgain"
...
If($InFile -ne "false")
{
   ...
   {
      Write-Host "* error: file not found: '$InFile'" -ForegroundColor Red -BackgroundColor Black
      Write-Host "";exit #Exit @Encrypt-String   
   }
   If($InFile -iNotMatch '(.ps1|.psm1|.psd1|.txt)$')
   {
      Write-Host "* error: accepted file formats: '.ps1|.psm1|.psd1|.txt'" -ForegroundColor Red -BackgroundColor Black
      Write-Host "";exit #Exit @Encrypt-String 
   }
