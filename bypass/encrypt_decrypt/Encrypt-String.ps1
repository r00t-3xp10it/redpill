...
# DESCRIPTION: last byte keys random function.
If($RandomByte -lt 240 -or $RandomByte -gt 255)
{
   #Randomize secret key last byte
...
If($Action -ieq "console")
{
   ...
   If($Deldecrypt -ieq "True")
   {
      ...
      write-host " Deldecrypt parameter has no use here." -ForegroundColor DarkGray;
   }

   If($RunElevated -ieq "True")
   {
      write-host "*" -ForegroundColor Red -NoNewline;
      write-host " RunElevated parameter has no use here." -ForegroundColor DarkGray;
   }
