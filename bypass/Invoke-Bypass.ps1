...
If($ExcludeLocation -ne "false")
{
   ...
   If($bool)
   {
      If(Test-Path -Path "$ExcludeLocation")
      {
         Set-MpPreference -ExclusionPath "$ExcludeLocation" -Force
         If((Get-MpPreference).ExclusionPath)
         {
            write-host "  + " -ForegroundColor DarkYellow -NoNewline;
         ...
         Else
         {
            write-host "  + Error: fail to add exclusion path .." -ForegroundColor Red -BackgroundColor Black;                                           
         }
      }
      Else
      {
         write-host "  + Error: not found '$ExcludeLocation'" -ForegroundColor Red -BackgroundColor Black;         
      }
   }
   Else
   {
...
## TODO:
# shanty function = comment: Try{}Catch{} , comment: Write-Host "`n";exit
# write-host "ERRORCODE: $?" -ForegroundColor Black -BackgroundColor White
If($?)
{
