...
.EXAMPLE
   ...
   Get string to convert to rot13 from text file.
...
If($text -eq "whoami" -and $InFile -ne "off")
{
   If(-not(Test-Path -Path "$InFile" -EA SilentlyContinue))
   {
      Write-Host "ERROR: -infile [< $InFile >] parameter input, not found .." -ForegroundColor Red -BackgroundColor Black
      Start-Sleep -Seconds 2;exit #Exit @enc.rot13 
   }

   #Get the cmdline\string to transform from text file!
   $text = Get-Content -Path "$InFile" -EA SilentlyContinue
}
