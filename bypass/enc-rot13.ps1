...
.NOTES
   ...
   Remark: -infile [<string>] only converts powershell scripts to rot13 chyper.
...
.EXAMPLE
   ...
   Get the commands to convert to rot13 from text file.

.EXAMPLE
   PS C:\> .\enc-rot13.ps1 -infile "cmdline.ps1"
   Get the sourcecode to convert to rot13 from script file.
...
If($InFile -ne "off")
{
   ...
   .SYNOPSIS
      Author: r00t-3xp10it
      Helper - Get sourcecode to transform from PS1 script!

   .NOTES
      This function allow attackers to converts the contents of -infile 'script.ps1'
      into rot13 string and builds the decrypt function to execute the sourcecode .
   #>
   ...
   If($InFile -iMatch '(.ps1)$')
   {
      #Get the cmdline\string to convert from ps1 script file!
      [string]$text = [System.IO.File]::ReadAllText("$InFile") -replace '\$','`$' -replace '"','`"'
      $Countlines = ($text | measure -line).Lines.ToString()
   }
   Else
   {
      #Get the cmdline\string to convert from text file!
      [string]$Rawtext = [System.IO.File]::ReadAllText("$InFile")
      If($Rawtext -Like '*$*')
      {
         Write-Host "`n`nerror: This function can not convert (`$)variable declarations." -ForegroundColor Red -BackgroundColor Black
         Write-Host "       => To convert PS variables put your code inside an script.ps1" -ForegroundColor Yellow
         Write-Host "       => and use -infile [<string>] to convert sourcecode to rot13`n" -ForegroundColor Yellow
         exit #Exit @enc-rot13
      }

      $Countlines = ($Rawtext | measure -line).Lines.ToString()
      #Replace new-line chars (\r\n) and escape any double quotes found!
