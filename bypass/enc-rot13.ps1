...
.NOTES
   ...
   or creates one PS1 script (%TMP%) with rot13 decrypt\exec function. -infile [<script.ps1|text.txt>]
   can be used to get the commands to be converted to rot13 from inside an existing file\script.ps1 
...
If($InFile -ne "off")
{
   ...
   {
      #Get the cmdline\string to convert to rot13 from ps1 script file!
      [string]$text = ([System.IO.File]::ReadAllText("$InFile")) -replace '\$','`$' -replace '"','`"'
      $Countlines = ($text | measure -line).Lines.ToString()
   }
   ElseIf($InFile -iMatch '(.txt)$')
   {
      #Get the cmdline\string to convert to rot13 from text file!
      [string]$Rawtext = [System.IO.File]::ReadAllText("$InFile")
      If($Rawtext -Like '*$*' -and $Output -iMatch '(ps1)$')
      {
         ...
         Write-Host "       => and use -infile [<file.ps1>] to convert sourcecode to rot13" -ForegroundColor Yellow
         Write-Host "          => syntax: .\enc-rot13.ps1 -infile 'test.ps1' -output ps1`n" -ForegroundColor Blue
         exit #Exit @enc-rot13
      }

      ##Parse data (convert to rot13 array)
      $Countlines = ($Rawtext | measure -line).Lines.ToString()
      #Replace new-line chars (\r\n) and escape any double quotes found!
      $ParseData = $Rawtext -replace "\r\n",";" -replace '"','`"' -replace '\$','`$' ##<- Create oneliner from input file!
