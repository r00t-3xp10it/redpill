...
[int]$MhsfsCounter = 0
    ...
    If($PingSweep -ieq "Enum" -or $PingSweep -ieq "Verbose"){## Loop function {Sellected Range}
        Write-Host "Range[${StartRange}..${EndOfRange}] Active IP Address on Local Lan!" -ForegroundColor Green
    ...
    If($PingSweep -ieq "Verbose"){
        ...
        Start-Sleep -Seconds 3;Clear-Host;Clear-Host
        Write-Host "Range[${StartRange}..${EndOfRange}] Active IP Address on Local Lan!" -ForegroundColor Green
        Write-Host "---------------------------------------------"
