...
If($Action -ieq "Enum" -or $Action -ieq "PortScan")
{
   ...
   $PrincipalInfo = Get-NetAdapter | Select-Object Name,Status,LinkSpeed,
      @{Name='Action';Expression={"$displayaction"}},
      ...
      $ping.Send("$IpRange") | Where-Object {
         $_.Status -Match 'Success'
            ...
            Else
            {
               ...
               $StatusCodeInf = (Get-WmiObject Win32_PingStatus -Filter "Address='$IpRange' and Timeout=100").StatusCode
   ...
   If($Action -ieq "PortScan")
   {
      ...
      {
         [int]$TotalNumberOfPortsToScan = "8"
         $PortRangerdse = @(#Port numbers to be scanned!
      ...
      Else
      {
         [int]$TotalNumberOfPortsToScan = "15"
      ...
      ForEach($Item in $DataBase)
      {
         ...
         If($FirstBannerDisplay -ieq "True")
         {
            ...
            If($AddPort -ne "false"){[int]$TotalNumberOfPortsToScan = [int]$TotalNumberOfPortsToScan+1}
         ...
         $ReplySizeInfo = (Get-WmiObject Win32_PingStatus -Filter "Address='$Item' and Timeout=100").ReplySize
         $TimeToLiveInf = (Get-WmiObject Win32_PingStatus -Filter "Address='$Item' and Timeout=100").TimeToLive
         $StatusCodeInf = (Get-WmiObject Win32_PingStatus -Filter "Address='$Item' and Timeout=100").StatusCode
