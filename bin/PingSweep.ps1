...
.NOTES
   ...
   Remark: By default windows waits for icmp packets reply for 1 second
   and drops the packet after timeout reached, this setting allow users
   to increase the icmp packets reply timeout (dont set less than 1000).
...
.Parameter TimeOut
   Icmp packets drop timeout (default: 1000)
...
.EXAMPLE
   ...
   Enum all addr + open ports (16 ports) + resolve Dns-NameHost!

.EXAMPLE
   PS C:\> .\PingSweep.ps1 -Action PortScan -ScanType bullet
   Enum all addr + open ports (9 ports) + resolve Dns-NameHost!

.EXAMPLE
   PS C:\> .\PingSweep.ps1 -Action PortScan -ScanType maxports -OutPut verbose
   Enum all addr + open ports (30 ports) + resolve Dns-NameHost + verbose outputs!
...
If($Action -ieq "Enum" -or $Action -ieq "PortScan")
{
      ...
      $ping.Send("$IpRange",$TimeOut) | Where-Object {
         $_.Status -Match 'Success'
            ...
            Else
            {
               ...
               {
                  $StatusCodeInf = "Destination Host Unreachable?"         
               }
               Else
               {
               ...
               Write-Host "   Inactive : $IpRange - StatusCode: $StatusCodeInf" -ForegroundColor Red -BackgroundColor Black
               exit #Exit @PingSweep
            }
              ...
              If($_.Status -Match 'Success')
              {
                 ...
                 If($OutPut -ieq "verbose")
                 {
                    ...
                    Else
                    {
                       ...
                       Write-Host "    |_" -ForegroundColor Yellow -NoNewline;Write-Host "Substract result from database and Id identifier." -ForegroundColor Red
                       Start-Sleep -Milliseconds 600
                    }
                 }
              }
   ...
   If($Action -ieq "PortScan")
   {
      ...
      {
         [int]$TotalNumberOfPortsToScan = "30"
         $PortRangerdse = @(#Port numbers to be scanned!
            "20,21,22,23,53,71,80,107,110,111,135,137,139,443,445,666,995",
            "1080,1433,1511,2049,3001,3128,3306,3389,5222,5900,6868,7878,8080"
         )
      }
      ElseIf($ScanType -ieq "Bullet")
      {
         [int]$TotalNumberOfPortsToScan = "9"
         $PortRangerdse = @(#Port numbers to be scanned!
      ...
      Else
      {
         [int]$TotalNumberOfPortsToScan = "16"
         #Topports sellection { default value }
         $PortRangerdse = @(#Port numbers to be scanned!
            "21,22,23,53,80,110,135,137,139,443,445,666,3306,5900,8080"
         )
      }
