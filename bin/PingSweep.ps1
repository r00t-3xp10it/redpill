...
[int]$MhsfsCounter = 0
    ...
    If($PingSweep -ieq "Enum" -or $PingSweep -ieq "Verbose")
    {
        ...
        $StartRange..$EndOfRange|ForEach-Object {## Loop truth ip adress(s) ranges
            $ping.Send("${DnSRaw}${_}")|Where-Object {## Send icmp packet
                ...
                If($_.Status -Match 'Success'){Write-Host "Active  :" $_.Address.ToString() -ForegroundColor Green}
        ...
        If(Test-Path -Path "$Env:TMP\iprange.log"){
            $Database = Get-Content -Path "$Env:TMP\iprange.log"|findstr /C:"Address"
    ...
    If($PingSweep -ieq "Verbose"){
        ...
        Get-Content -Path "$Env:TMP\BrowserEnum.log" | Format-List |
           Out-String -Stream | ForEach-Object {
              $stringformat = If($_ -iMatch '(Open|Status)')
              {
