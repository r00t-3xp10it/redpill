...
If($Action -ieq "Enum" -or $Action -ieq "PortScan")
{
   ...
   If($logfile -ieq "True")
   {
      ...
      If($Action -ieq "Enum")
      {
         ...
         Write-Host "'$Env:TMP\$Rand.log'" -ForegroundColor Green -BackgroundColor Black -NoNewline;
      }
   }
   Write-Host ""
   ...
   If($Action -ieq "PortScan")
   {
      ...
      ForEach($Item in $DataBase)
      {
         ...
         Write-Host "replycod: $StatusCodeInf replysize:$ReplySizeInfo ttl:$TimeToLiveInf" -ForeGroundColor Yellow    
