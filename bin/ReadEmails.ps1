...
If($Action -ieq "Contacts")
{
   ...
   $ContactsInfo | Format-List ShowItemCount,CustomViewsOnly,DefaultMessageClass,AddressBookName,@{Name='Items';Expression={''}} |
   Out-String -Stream | Select-Object -Skip 2 | ForEach-Object {
...
If($Logfile -ieq "True")
{
   ...
   If($Action -ieq "Contacts")
   {
      echo $ContactsInfo >> $LogName
      echo $Contacts >> $LogName
   }
   Else
   {
