...
.Parameter EmailsCount
   How many outlook ite objects to display (default: 5)
...
If($Action -ieq "Contacts")
{
   ...
   Else
   {
      ...
      $TerMinalOutPut | Select-Object FullName,CompanyName,Department,JobTitle,Anniversary,Birthday,Email1Address,BusinessAddressCity,HomeAddressStreet,HomeAddressCity,HomeAddressState,HomeTelephoneNumber | Format-list | Out-String -Stream | Select-Object -Skip 2
   }
}
...
If($Action -ieq "Enum")
{
   ...
   If($verb -ieq "True")
   {
      If($Filter -ne "False")
      {
         If($EmailsCount -gt 10){$EmailsCount = "10"} #Limmit the search to 10 max Objects for fast reports
   ...
   Else
   {
      If($Filter -ne "False")
      {
         If($EmailsCount -gt 10){$EmailsCount = "10"} #Limmit the search to 6 Objects for fast reports
