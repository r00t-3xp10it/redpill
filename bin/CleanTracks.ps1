...
If($CleanTracks -ieq "Clear" -or $CleanTracks -ieq "Paranoid")
{
   ...
   If($CleanTracks -ieq "Paranoid")
   {
      ...
      $DirectoryToFilter = "$Env:WINDIR" + "\Sys" + "tem32" -join ''
      ...
      powershell -File "$Env:TMP\Lnk-Sweeper.ps1" -Action "clean" -PSBanner "false" -Directory "$DirectoryToFilter" -TimeStamp "today"
