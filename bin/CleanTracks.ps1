...
.OUTPUTS
   ...
   [-] Cleaning: Recycle bin folder artifacts.
       verbose : Cleaned all recycle bin artifacts.
...
If($CleanTracks -ieq "Clear" -or $CleanTracks -ieq "Paranoid")
{
   ...
   If(Get-Command "Clear-RecycleBin" -EA SilentlyContinue)
   {
      ...
      If($verb -ieq "True")
      {
         [int]$CountDeletedLines = ((New-Object -ComObject Shell.Application).NameSpace(0x0a).Items()|Measure-Object -Line).Lines
         Write-Host "    verbose : Cleaned '$CountDeletedLines' recycle bin artifacts."
         $Count = $Count+$CountDeletedLines
      }
