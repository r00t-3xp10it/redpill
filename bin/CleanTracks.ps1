...
If($CleanTracks -ieq "Clear" -or $CleanTracks -ieq "Paranoid")
{
   ...
   If($Verb -ieq "True"){$fdx = "-"}Else{$fdx = "*"}
   Write-Host "[$fdx] Cleaning: Temporary folder artifacts." -ForegroundColor Blue
   ...
   {
      $Count = $Count+1
      Write-Host "[*] Cleaning: ConsoleHost_history records." -ForegroundColor Blue
   ...
   If($CleanTracks -ieq "Paranoid")
   {
      ...
      If($Verb -ieq "True"){$fdx = "-"}Else{$fdx = "*"}
      Write-Host "[$fdx] Cleaning: StartUp directory artifacts." -ForegroundColor Blue
      ...
      If($IsClientAdmin)
      {
         ...
         Write-Host "[-] Cleaning: LogFiles from Eventvwr snapin." -ForegroundColor Blue
         If($Verb -ieq "False")
         {
