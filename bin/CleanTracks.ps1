...
If($CleanTracks -ieq "Clear" -or $CleanTracks -ieq "Paranoid")
{
   ...
   Write-Host "  => Clear: Multiple locations\regedit." -ForegroundColor Blue
   ...
   If($CleanTracks -ieq "Paranoid")
   {
      ...
      If($IsClientAdmin)
      {
         ...
         Write-Host "  => Clear: ALL LogFiles from Eventvwr snapin." -ForegroundColor Blue
         $PSlist = wevtutil el | Where-Object {
            $_ -iNotMatch '(Microsoft-Windows-LiveId/Analytic|Microsoft-Windows-LiveId/Operational|Microsoft-Windows-USBVideo/Analytic|/Admin)$'
         }
