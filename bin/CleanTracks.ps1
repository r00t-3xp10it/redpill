...
If($CleanTracks -ieq "False" -or $CleanTracks -ieq $null)
{
   ...
   Start-Sleep -Milliseconds 3700;Get-Help .\CleanTracks.ps1 -full;exit #Exit @CleanTracks
}
...
If($CleanTracks -ieq "Clear" -or $CleanTracks -ieq "Paranoid")
{
   ...
   If($verb -ieq "True")
   {
      Write-Host "    verbose : Done cleaning common locations and regedit."
   }
   ...
   Write-Host "[$Char] Cleaning: Jumplists folder artifacts." -ForegroundColor Blue;Start-Sleep -Milliseconds 300
   ...
   If(-not($CleanPSLogging -ieq $null))
   {
      ...
      If($Verb -ieq "True"){Start-Sleep -Milliseconds 300;Write-Host "    verbose : nothing-to-see-here"}
   }
   ...
   If($CleanTracks -ieq "Paranoid")
   {
      ...
      Else
      {
         Write-Host "    Warning : Administrator privileges required." -ForegroundColor Red -BackgroundColor Black
      }
