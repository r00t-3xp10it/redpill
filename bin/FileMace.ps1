...
If($FileMace -ne "false")
{
    ...
    {
        Write-Host "`n   [missing]: $FileMace" -ForegroundColor Red
        return
     }

     ## Make sure input its NOT a directory - Attributes
     If((Get-ChildItem -Path "$FileMace").Attributes -iNotMatch 'Archive')
     {
        Write-Host "`n   Error: this module does NOT modify directorys." -ForegroundColor Red
        return     
     }
