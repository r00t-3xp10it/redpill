...
Else
{
   If($Clean -ieq "True")
   {
      ...
      {
         Write-Host "`nError: 'Clear-RecycleBin' command not found .." -ForegroundColor Red -BackGroundColor Black
         Write-Host "None items have been deleted from recycle bin ..`n" -ForegroundColor Blue
         Exit #Exit @list-recycle-bin         
      }
   
      Clear-RecycleBin -Force|Out-Null
      #Delete all recycle bin items (no stdout displays)
      Write-Host "`n* Successful deleted all items from recycle bin.`n" -ForegroundColor Green  
      Exit #Exit @list-recycle-bin
   }
