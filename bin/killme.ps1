...
If($bool)
{
   ...
   {
      write-host "   x Error: deleting extension from process name." -ForegroundColor Red
   ...
   Else
   {
      write-host "   x Error: Fail to stop process '$Proc_name'?" -ForegroundColor Red
   }
}
Else
{
   write-host "   x Error: Administrator privs required to kill processes." -ForegroundColor Red
}