...
Format-Table -AutoSize | Out-String -Stream | Select -SkipLast 1 | ForEach-Object {
...
try{
   #Prevent curl fail output displays!
   $PublicAddr = (curl ifconfig.me).Content
}catch{
   #[error] curl => failed to retrieve public IP address!
