...
If($Action -ieq "verbose")
{
   ...
   .NOTES
      ...
      and it also resolve DNS host names!
   #>
   ...
   try{#Retrieve Active NetAdapter statistics!
      ...
      #I needed to put the table inside an variable to be abble to build the logfile later on!
      ...
      echo $displayStatisticsTable | Format-Table -AutoSize | Out-String -Stream | ForEach-Object {
         $stringformat = If($_ -iMatch 'SystemName'){
            @{ 'ForegroundColor' = 'Green' } }Else{ @{ } }
         Write-Host @stringformat $_
      }
   }catch{
      Write-Host "[error] failed to retrieve NetAdapter statistics!`n" -ForeGroundColor Red -BackGroundColor Black;Start-Sleep -Seconds 1
   }

   #Resolve all host IP addresses to DNS names
   Get-NetTCPConnection -State ESTABLISHED | Select-Object -Property OwningProcess,RemoteAddress,@{name='DnsHostName';expression={(Resolve-DnsName $_.RemoteAddress).NameHost}} |
     Format-Table -AutoSize | Out-String -Stream | ForEach-Object {
         $stringformat = If($_ -iMatch 'DnsHostName'){
            @{ 'ForegroundColor' = 'Green' } }Else{ @{ } }
         Write-Host @stringformat $_
      }
}
