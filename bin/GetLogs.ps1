...
If($GetLogs -ieq "Yara")
{
   ...
   If($verb -ieq "system" -or $verb -ieq "application" -or $Verb -ieq "security")
   {
      ...
      ElseIf($Id -ieq "false")
      {
         ...
         {
            ## Default Id's to scan! (if none user inputs)
            # 3,4624,4625,4672,4688,4728  -> security
            $RawLit = "3,4624,4625,4672,4688,4728,4738"
            $IdList = $RawLit.Split(',')
         }
   ...
   Else
   {
      ...
      ElseIf($Id -ieq "false")
      {
         ...
         # ID: 4672,4728,4738      -> Security
         $RawLit = "1,59,60,104,300,403,800,1116,1117,4100,4104,4672,4728,4738,5007,5858,5861,8002,8004"
         $IdList = $RawLit.Split(',')
