...
If(-not(Test-Path -Path "$AddrBook" -EA SilentlyContinue))
{
   ...
   If($NetMaskConf -NotMatch '^(255.255.255.0)$')
   {
      ...
      write-host "   By default smblogin-spray.ps1 scans 255.255.255.0 net masks"
   ...
   For($i=$StartRange; $i -lt $EndRange; $i++)
   {
      If($Filter -ieq "active")
      {
         ...
         $IsAlive = $PingObject.Send("${RangeList}${i}", $PingTimeOut) | Where-Object { $_.Status -Match 'Success' }
         If($IsAlive){echo "${RangeList}${i}" | Out-File "$AddrBook" -Encoding ascii -Force -Append}
      }
      Else
      {
