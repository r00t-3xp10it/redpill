...
If($Egg -ieq "false")
{
   ...
   If($Filter -ieq "active")
   {
      ...
      write-host "' hosts will be added to list." -ForeGroundColor DarkGray;
   }
}
...
[int]$Counter = 0
$AddrBook = "$pwd\hosts.log" #List of addresses to scan.
If(-not(Test-Path -Path "$AddrBook" -EA SilentlyContinue))
{
   ...
   .NOTES
      ...
      Remark: -PingTimeOut parameter accepts values from 200 Milliseconds to 1500 Milliseconds. 
   ...
   #Get Net Mask Configuration tests
   ...
   $EndRange = $EndRange+1
   If($Filter -ieq "active"){write-host ""}
   If($PingTimeOut -lt 200 -or $PingTimeOut -gt 1500)
   {
      [int]$PingTimeOut = 500 # 500 ms its the default value
   }
...
write-host "   testing smb credentials .." -ForegroundColor Green
   ...
   try{
      ...
      Else
      {
         ...
         ElseIf($error[0].exception -iMatch 'Access is denied')
         {
            ...
            If($LogFile -ieq "True")
            {
               echo "[`$`$`$`$] ${Item}: '$DomainName,${Password}' [success,non-admin]" | Out-File "smblogin.results.log" -Encoding ascii -Append
            }
         }
         Else
         {
...
If($Counter -gt 0)
{
   ...
   write-host "'$Counter'`n" -ForegroundColor Green -NoNewline;
}
Else
{
   write-host " *" -ForegroundColor Red -NoNewline;
   write-host " Total of credentials found: " -ForegroundColor Blue -NoNewline;
   write-host "'$Counter'`n" -ForegroundColor DarkGray -NoNewline;
}