...
.NOTES
   ...
   mount the '\\TARGET\Admin$' share, it means that we have administrative privileges.
...
If(Test-Path -Path "$AddrBook" -EA SilentlyContinue)
{
...
[int]$Counter = 0
$EndRange = $EndRange+1
## Builds the required hosts.txt file
...
For($i=$StartRange; $i -lt $EndRange; $i++)
{
   ...
   .SYNOPSIS
      Author: @r00t-3xp10it
      Helper - Creates hosts.log dicionary with local host addresses.
   #>
      ...
      Else
      {
         ...
         Else
         {
            ...
            If($Item -match '.(72)$')
            {
               ...
               If($LogFile -ieq "True")
               {
                  echo "[cred] ${Item}: '$DomainName,$Password' [success]" | Out-File "smblogin.results.log" -Encoding ascii -Append
               }
            }
            Else
            {
