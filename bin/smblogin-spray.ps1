...
If($Force -ieq "false")
{
   ...
   $TestService = (Get-Service -Name "LanmanWorkstation").Status
...
If(-not(Test-Path -Path "$AddrBook" -EA SilentlyContinue))
{
   ...
   If($NetMaskConf -NotMatch '^(255.255.255.0)$')
   {
      ...
      write-host "   ip addressses to be scanned inside of hosts.log logfile and"
   ...
   For($i=$StartRange; $i -lt $EndRange; $i++)
   {
      If($Filter -ieq "active")
      {
         ...
         $IsAlive = $ping.Send("${RangeList}${i}", $PingTimeOut) | Where-Object {$_.Status -Match 'Success'}
         If($IsAlive)
         {
