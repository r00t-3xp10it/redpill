...
If($Action -ieq "verbose")
{
   ...
   #Driver Check
   $Result = Switch ((Get-ChildItem -Path "$Env:SystemDrive\Windows\System32\drivers" | Where-Object  Name -Match '(.sys)$').name)
   {
      ...
      WdFilter.sys           {Obj @{Driver=$_;EDR= 'Windows Defender AV'      }}
      hvsifltr.sys           {Obj @{Driver=$_;EDR= 'Windows Defender AV'      }}
      WdNisDrv.sys           {Obj @{Driver=$_;EDR= 'Windows Defender AV'      }}
   }
   ...
   Else
   {
      Return $Result | Format-Table -AutoSize | Out-String -Stream | ForEach-Object {
