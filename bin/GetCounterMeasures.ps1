<#
.SYNOPSIS
   List common security processes runing!

   Author: r00t-3xp10it
   Tested Under: Windows 10 (19042) x64 bits
   Required Dependencies: Get-Process {native}
   Optional Dependencies: Get-MpPreference {native}
   PS cmdlet Dev version: v2.2.18

.DESCRIPTION
   This cmdlet enumerates most common security processes runing (PS)
   AppWhitelisting, Behavioral Analysis, Intrusion Detection, Firewall
   Defender preferences, DEP, DLP, HIPS, Hunt for EDR's by driver names.

.NOTES
   Invoking -action 'verbose' triggers cmdlet to hunt for common security
   processes running, AppWhitelisting, Behavioral Analysis, Intrusion Detection
   Firewall, Defender preferences, DEP, DLP, HIPS, Hunt for EDR's by driver name.

   Invoking -stringsearch 'true' -string 'Defender|antimalware' arguments triggers
   cmdlet to recursive search for strings 'Defender|antimalware' on file description.
...
.Parameter String
   ...
   Remark: To split diferent strings to search use the PIPE (|) command
...
.EXAMPLE
   ...
   List common security products processes runing!
...
.EXAMPLE
   ...
   Search for strings 'Defender|antimalware' on drivers FileDescription 

.EXAMPLE
   PS C:\> .\GetCounterMeasures.ps1 -stringsearch 'true' -string 'Defender'
   Search for string 'Defender' on drivers FileDescription 

.EXAMPLE
   PS C:\> .\GetCounterMeasures.ps1 -action 'verbose' -stringsearch 'true' -string 'Security'
   Trigger cmdlet full scan + string search (security) on drivers file description
...
If($StringSearch -ieq "true")
{
   ...
   .SYNOPSIS
      Author: @r00t-3xp10it
      Helper - Hunt for strings on driver.sys file description
   ...
   ForEach($ItemFound in $FinalDrivers)
   {
      ...
      ElseIf($ItemFound -iMatch '^(edrsensor.sys)$')
      {
      ...
      ElseIf($ItemFound -iMatch '^(mbamwatchdog.sys)$')
      {
      ...
      ElseIf($ItemFound -iMatch '^(ssfmonm.sys)$')
      {
         $EDRreport = "Webroot Software"      
      }
      Else
      {
         $EDRreport = "Unknow Product"      
      }
