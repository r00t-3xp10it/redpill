...
If($ADS -ieq "Enum")
{
    ...
    If($StreamData -ieq "false")
    {
        ...
        Get-Content -Path "$Env:TMP\jdgfjd.log";Remove-Item -Path "$Env:TMP\jdgfjd.log" -Force
        Write-Host "[error] -streamdata " -ForegroundColor Red -BackgroundColor Black -NoNewline;
        Write-Host "'string'" -ForegroundColor DarkGray -BackgroundColor Black -NoNewline;
        Write-Host "mandatory parameter required!" -ForegroundColor Red -BackgroundColor Black;
...
ElseIf($ADS -ieq "Exec")
{
    ...
    If(-not(Test-Path -Path $InTextFile -ErrorAction SilentlyContinue))
    {
        ...
        Get-Content -Path "$Env:TMP\kbfdjk.log";Remove-Item -Path "$Env:TMP\kbfdjk.log" -Force
...
ElseIf($ADS -ieq "Create")
{
    ...
    Else
    {
       ...
       Get-Content -Path "$Env:TMP\gdppdi.log";Remove-Item -Path "$Env:TMP\gdppdi.log" -Force
       Write-Host "[error] Not found: $InTextFile `n`n" -ForegroundColor Red -BackgroundColor Black
       exit ## Exit @AdsMasquerade
    }
    ...
    If(Test-Path $InTextFile -EA SilentlyContinue)
    {
        ...
        If(Test-Path -Path "$Env:TMP\gfscgsvs.log" -EA SilentlyContinue)
        {
            ...
            Else
            {
                ...
                Get-Content -Path "$Env:TMP\lsctt.log";Remove-Item -Path "$Env:TMP\lsctt.log" -Force
