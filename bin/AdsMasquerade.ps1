...
If($ADS -ieq "Enum")
{
    ...
    .EXAMPLE
        .\AdsMasquerade.ps1 -ADS Enum -StreamData ":`$DATA" -StartDir "$Env:USERPROFILE\Desktop"
        Search recursive for payload.bat ADS stream record existence, starting on -StartDir [ dir ]
    ...
    try{#Start Query
       Get-ChildItem "$StartDir" -Recurse -Force -EA SilentlyContinue | % { Get-Item $_.FullName -stream * } |
       Where-Object Stream -ieq "${FinalQuery}" | Where-Object { $_ -iNotMatch 'Zone.Identifier' -and $_.FileName -iNotMatch '(.dat|.lock)$'} |
