...
.Parameter ScanType
   Increase scan success rate [false_positives] (default: normal)

.Parameter Verb
   Switch to diplay the file contents?
...
.EXAMPLE
   PS C:\> .\Find-strings.ps1 -Path "$Env:TMP" -scantype hard
   Increase scan success rate [hard] (increase false_positives)
...
If($ScanType -iNotMatch '^(normal)$')
{
   ## Increase scan success rate - but also increases false positives displays
