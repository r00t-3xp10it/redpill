...
If($String -Match '(0\)n\.\{0,2\}\(=|:\)\.\[\^\$\]\{0,22\}\$\))$')
{
   ...
   $String = "$String" + "|((cp|p)ass.{0,4}`":.{0,22}$)|(^Password(\s:|:))|(`"(Pass.{0,5}|User.{0,4})`">)|(Username`":.{0,22}$)|([^$]Auth(_+t|t)oken(\s*|\s+`"|`"|`"+:|`"+>|\s+:|:|\s+=|=))" -join ''
}
...
If($AllPossible.IsPresent)
{
   ...
   $String = "(^(\s+U|U)ser.{0,4}(\s+`"|`"|`"+:|`"+>|\s+:|:|\s+=|=).[^$]{0,150}$)|(^(\s+p|\s+cp|cp|p)ass.{0,4}(\s+`"|`"|`"+:|`"+>|\s+:|:|\s+=|=).[^$])|(^(\s+p|p)wd(\s+`"|`"|`"+:|`"+>|\s+:|:|\s+=|=).[^$]{0,150}$)|(log(i|o)n(\s+`"|`"|`"+:|`"+>|\s+:|:|\s+=|=).[^$]{0,150}$)|(Username(\s+`"|`"|`"+:|`"+>|\s+:|:|\s+=|=).{0,150}$)|(Password(\s+`"|`"|`"+:|`"+>|\s+:|:|\s+=|=))|([^$]Auth(_+t|t)oken(\s*|\s+`"|`"|`"+:|`"+>|\s+:|:|\s+=|=))" 
}
Else
{
