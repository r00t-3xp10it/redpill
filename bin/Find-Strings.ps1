...
.NOTES
   ...
   Use | (pipe) command to split exclusion extensions (eg .txt|.ini|.bat)
     
.Parameter Path
   The directory where to start search recursive (default: $Env:USERPROFILE)

.Parameter String
   String or Regex (default: ^(\s+U|U)ser.{0,6}(=|:)|^(\s+p|\s+cp|cp|p)ass.{0,6}(=|:)|login.{0,2}(=|:))
...
[CmdletBinding(PositionalBinding=$false)] param(
   [string]$String="^(\s+U|U)ser.{0,6}(=|:)|^(\s+p|\s+cp|cp|p)ass.{0,6}(=|:)|login.{0,2}(=|:)",
...
If($i -lt 1)
{
   write-host "   Error: fail to find any strings inside current directory.`n`n" -ForegroundColor Red
}