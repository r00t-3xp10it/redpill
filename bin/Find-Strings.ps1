...
If($Verb -iNotMatch '^(false)$')
{
   ...
   If((Get-Service -Name "VaultSvc").Status -Match '^(Running)$')
   {
      ...
      {
         write-host "`n     - [Vault] " -ForegroundColor Red -NoNewline
         write-host "None plain text credentials have been found." -ForegroundColor DarkGray
      }
      Else
      {           ## TODO: -SkipLast 2 (test the output)
         ## Print OnScreen resource_names + credentials (plain text) found
         $vault.RetrieveAll() | % { $_.RetrievePassword(); $_ } | Select-Object -Property UserName,Resource,Password |
            Format-Table -AutoSize | Out-String -Stream | Select-Object -SkipLast 1 | ForEach-Object {
               $stringformat = If($_ -Match '^(UserName)')
               {
                  @{ 'ForegroundColor' = 'Green' }
               }
               ElseIf($_ -iMatch "$Resource")
               {
