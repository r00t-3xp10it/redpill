...
.LINK
   https://github.com/r00t-3xp10it/Red_Team
   https://github.com/r00t-3xp10it/Red_Team/Exfiltration/DLLSearch.ps1
#>
...
ForEach($Item in $RunningProcesses)
{
   ...
   $PPID = (Get-Process -Name $Item|Select -Last 1).Id
   $PPATH = (Get-Process -Name $Item|Select -Last 1).Path
   ...
   $DllTable.Rows.Add("$PPID","$Item","$PPATH","$Parse")|Out-Null
}
