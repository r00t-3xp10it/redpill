...
.NOTES
   ...
   the current powershell console than invoke -exclude "$pid"
...
.EXAMPLE
   PS C:\> .\killProcess.ps1 -proc_name 'powershell' -exclude "$pid"
...
If($IsAdmin)
{
   ...
   Else
   {
      ...
      ForEach($Token in $PLIST)
      {
         ...
         Start-Process -WindowStyle Hidden Powershell -ArgumentList "Get-Process -Name `"$Token`"|Where-Object{`$_.Id -ne `"$Exclude`"}|Stop-Process -Force" -Wait   
      }

      If($MYPID -Match "$Exclude")
      {
         write-host "   Process State : " -NoNewline
         write-host "$Proc_name stopped except for PID $Exclude" -ForegroundColor Green
         write-host "   Process Path  : $PPATH"
         return      
      }
