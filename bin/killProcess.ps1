...
If($IsAdmin)
{
   ...
   Else
   {
      ...
      {
         write-host "   Error: deleting extension from process name." -ForegroundColor DarkGray
      ...
      If($DontKill -NotMatch '^\d+$')
      {
         ...
         write-host "   Error: wrong input, default dontkill to $DontKill" -ForegroundColor DarkGray
      }
