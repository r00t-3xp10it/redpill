<#
.SYNOPSIS
   Use this cmdlet to host files for download. The idea of this is to have a
   PowerShell SimpleHTTPServer that is similar to Python's SimpleHTTPServer module.

.DESCRIPTION
   Running this function will open a PowerShell web server hosting files in the current
   directory. The server can be accessed at http://localhost:8000 You can download files.
   The directories are not able to be traversed through the web server.
