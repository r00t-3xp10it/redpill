...
If($History.IsPresent)
{
   ...
   .SYNOPSIS
      Author: @r00t-3xp10it
      Helper - Dump installed browsers URL history
   #>
   ...
   If(-not(Test-Path -Path "$BraveHistory"))
   {
      New-Object -TypeName PSObject -Property @{
         Browser = "Brave"
         UrlHistory = "None history entrys found."
   ...
   Else
   {
      $Value = Get-Content -Path "$BraveHistory"|Select-String -AllMatches $Regex |% {($_.Matches).Value} |Sort -Unique 
      $Value | ForEach-Object { 
         $Key = $_ 
         If($Key -Match $Search)
         { 
...
while($true)
{
   ...
   .SYNOPSIS
      Author: @r00t-3xp10it
      Helper - Loop funtion to retrieve browser windows titles.
   #>
