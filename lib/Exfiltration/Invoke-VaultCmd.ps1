...
ForEach($Token in $Schemas)
{
   ...
   $Names = vaultcmd /listcreds:"$Token" /all | Select-String -Pattern 'Resource' | ForEach-Object {
      $Noia = VaultCmd /listproperties:"$Token" | findstr "Current protection method:" | findstr /V "Location"
      $parseOne = $Noia -replace 'Current protection method: ',''
      $Nubi = VaultCmd /listproperties:"$Token" | findstr "Vault Properties:" | findstr /V "Location"
      $parseTwo = $Nubi -replace 'Vault Properties: ',''
      $ResourceName = $_ -replace 'Resource: ',''

      ## Add entrys found to Table
      $VaultEntrys.Rows.Add("$parseOne","$parseTwo","$ResourceName")|Out-Null
   }
}
...
If($Action -iMatch '^(check)$')
{
   ...
   If(($vault.RetrieveAll()) -ne $null)
   {
      ...
      $vault.RetrieveAll() | Select-Object Username,Resource,@{Name='Password Type';Expression={"Hidden::Generic"}} |
         Out-String -Stream | Select-Object -SkipLast 1 | ForEach-Object {
...
If($Action -iMatch 'create')
{
   ...
   write-host "[vault] " -ForegroundColor Green -NoNewline
   write-host "Creating '$Resource' credential`n"
   ...
   Else
   {
      ...
      $vault.RetrieveAll()|Select-Object -Property UserName,Resource,
         @{Name='Password Type';Expression={"Hidden::Generic"}} | Out-String -Stream |
...
If($Action -iMatch 'Dumpall')
{
   ...
   $vault.RetrieveAll() | % { $_.RetrievePassword(); $_ } |
      Select-Object -Property UserName,Resource,Password |
...
If($Action -iMatch 'delete')
{
   ...
   Else
   {
      ...
      $Results | Out-String -Stream | ForEach-Object {
         $stringformat = If($_ -Match '^(UserName)')
         {
            @{ 'ForegroundColor' = 'Green' }
         }
