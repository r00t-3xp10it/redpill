...
.DESCRIPTION
   ...
   to decode on Password vault, create credentials with Generic
   or PSCredential passwords, dumps all generic or created PSCredentials
   creds, dumps DPAPI master keys + blobs keys + web browsers login files
   full path and delete credentials (resource) from windows password vault.
...
.NOTES
   ...
   in plain text to PSCredential and stores one .secret file with the
...
If($Action -iMatch '^(create)$')
{
   ...
   If($Secure.IsPresent)
   {
      ...
      .NOTES
         Parameter -secure switch allow us to modify our generic password
         in plain text to PSSecureString and stores one .secret file with
         PSSecure string for this cmdlet decode it later if its required.
      #>
      ...
      ## creation of .secret filename
      $encryptedpwd = $securepwd | ConvertFrom-SecureString
      $FINANAE = $Resource -replace '[^a-z 0-9]','' ## .secret FileName 
      $RandNom = -join (((48..57)+(65..90)+(97..122)) * 80 |Get-Random -Count 4 |%{[char]$_})
      $LogFileName = "$FINANAE" + "_" + "$RandNom" + ".secret" -join ''

      ## Send to .secret file
      echo "Creation : $(Get-Date -Format 'dd/MM/yyyy::HH:mm:ss')" > "$Env:LOCALAPPDATA\$LogFileName"
      echo "UserName : $UserName" >> "$Env:LOCALAPPDATA\$LogFileName"
      echo "Resource : $Resource" >> "$Env:LOCALAPPDATA\$LogFileName"
      echo "Password : $encryptedpwd" >> "$Env:LOCALAPPDATA\$LogFileName"
   }
...
If($Action -iMatch '^(dump)$')
{
   ...
   If($Secure.IsPresent)
   {
      ...
      .SYNOPSIS
         Author: @r00t-3xp10it
         Helper - Search for invoke-vaultcmd stored PScredential .secret files

      .OUTPUTS
         UserName     Resource                                    Password
         --------     --------                                    --------
         SKYNET\pedro https://www.siliconvalley/classified.portal 01000000d08c9ddf0115d1118c7a00c04fc297eb010...

         :Decoding PScredentials:
         [USERNAME]::SKYNET\pedro
         [PASSWORD]::r00t3xp10it
         [CREATION]::15/01/2023::12:02:28 
         [RESOURCE]::https://www.siliconvalley/classified.portal
         [CREDPATH]::C:\Users\pedro\AppData\Local\httpswwwsiliconvalleyclassifiedportal_A7oE.xml
         [ PBDATA ]::01000000d08c9ddf0115d1118c7a00c04fc297eb010000008c5a0c5f6bbaef4499136b38cb146eef000000000
      ...
      ## Search for invoke-vaultcmd stored PScredential .secret files
      $CheckStoredCreds = (Get-ChildItem -Path "$Env:LOCALAPPDATA").FullName|Where{$_ -iMatch "(_.{4}.secret)$"}
      ## Loop trougth all files found
      ForEach($Token in $CheckStoredCreds)
      {
         ## Get Information from .secret file
         $GetDate = (Get-Content -Path "$Token" | Select-String -Pattern "Creation") -replace 'Creation : ',''
         $GetSecret = (Get-Content -Path "$Token" | Select-String -Pattern "Password") -replace 'Password : ',''
         $GrabUserName = (Get-Content -Path "$Token" | Select-String -Pattern "UserName") -replace 'Username : ',''
         $NOExtension = (Get-Content -Path "$Token" | Select-String -Pattern "resource") -replace 'resource : ',''
         $securepwd = $GetSecret | ConvertTo-SecureString

         ## Decrypt PSCredential
         $Marshal = [System.Runtime.InteropServices.Marshal]
         $Bstr = $Marshal::SecureStringToBSTR($securepwd)
         $DecodeFunction = $Marshal::PtrToStringAuto($Bstr)
         ...
         write-host "[" -NoNewline
         write-host "CREATION" -ForegroundColor DarkGreen -NoNewline
         write-host "]::" -NoNewline
         write-host "$GetDate"
         write-host "[" -NoNewline
         ...
         write-host "[ " -NoNewline
         write-host "PBDATA" -ForegroundColor DarkGreen -NoNewline
         write-host " ]::$GetSecret`n"
      }
   }
}
...
If($Action -iMatch '^(delete)$')
{
   ...
   If($Secure.IsPresent)
   {
      ...
      SYNOPSIS
         Author: @r00t-3xp10it
         Helper - Delete Resource Name .secret PSCredential file
      #>

      $FINANAE = $Resource -replace '[^a-z 0-9]',''
      ## Search for invoke-vaultcmd stored PScredential .secret files
      $CheckStoredCreds = (Get-ChildItem -Path "$Env:LOCALAPPDATA").Name|Where-Object{$_ -iMatch "^(${FINANAE}_.{4}.secret)$" }
      If(-not([string]::IsNullOrEmpty($CheckStoredCreds)))
      {
...
write-host "  Get-Help .\Invoke-VaultCmd.ps1 -detailed" -ForegroundColor DarkGray
