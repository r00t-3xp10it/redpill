...
If($Action -iMatch '^(DPAPI)$')
{
   ...
   If($Dpapi1 -ne $null -or $Dpapi2 -ne $null)
   {
      ...
      $GuidMasterKeyId = (gci "$Location").Name
      $GuidMasterKey = "$Location"+"\${GuidMasterKeyId}" -join ''
      write-host "[GuidMasterKey] " -NoNewline
      write-host "$GuidMasterKey" -ForegroundColor Red
      write-host "[decode_hashes] " -NoNewLine
   ...
   Else
   {
      write-host "[vault] none DPAPI master keys found to decode?`n" -ForegroundColor Red
      return
   }
...
write-host "  Get-Help .\Invoke-VaultCmd.ps1 -full`n" -ForegroundColor DarkGray
exit
}