...
.DESCRIPTION
   Manage Windows Password Vault Items
   Check for valid credentials, create one credential
   [generic password], dump all generic password creds
   from vault in plain text, dump dpapi master keys +
   blobs and web browsers login files full path, and
   delete credentials from windows password vault.

.NOTES
   Parameter -resource 'name' and -username 'username'
   are mandatory requirements to delete entrys from Vault.
   Execute_on_console: 'Control Keymgr.dll,KRShowKeyMg'
   to manual access the windows credential manager (GUI)
...
If($Action -iMatch '^(check)$')
{
   ...
   If(($vault.RetrieveAll()) -ne $null)
   {
      ...
      $vault.RetrieveAll() | Select-Object Username,Resource,@{Name='Password Type';Expression={"Hidden::Generic"}} |
         ...
         {
            ## Append data to report logfile
            $vault.RetrieveAll() | Select-Object Username,Resource,@{Name='Password Type';Expression={"Hidden::Generic"}} >> "$pwd\Invoke-VaultCmd.report"
         }
   }
   Else
   {
   ...
   Else
   {
      ...
      $vault.RetrieveAll() | Where-Object { $_.Resource -iMatch "^($REsource)$" -and $_.UserName -iMatch "$titanio"} |
         ...
         If($Log.IsPresent)
         {
            ## Append data to report logfile
            $vault.RetrieveAll() | Where-Object { $_.Resource -iMatch "^($REsource)$" -and $_.UserName -iMatch "$titanio"} |
               Select-Object -Property UserName,Resource,@{Name='Password Type';Expression={"Hidden::Generic"}} >> "$pwd\Invoke-VaultCmd.report"
         }
   }
...
If($Action -iMatch '^(dump)$')
{
   ...
   $vault.RetrieveAll() | % { $_.RetrievePassword(); $_ } |
      ...
      If($Log.IsPresent)
      {
         ## Append data to report logfile
         $vault.RetrieveAll() | % { $_.RetrievePassword(); $_ } |
            Select-Object -Property UserName,Resource,Password >> "$pwd\Invoke-VaultCmd.report"
      }
...
If($Action -iMatch '^(DPAPI)$')
{
   ...
   If($Dpapi1 -ne $null -or $Dpapi2 -ne $null)
   {
      ...
      If($Log.IsPresent)
      {
         echo "`n[PATH]::$Env:APPDATA\Microsoft\Credentials" >> "$pwd\Invoke-VaultCmd.report"
   ...
   Else
   {
      ...
      If($Log.IsPresent)
      {
         echo "`n[SID ]::$UserName_SSID" >> "$pwd\Invoke-VaultCmd.report"
   ...
   ForEach($Item in $WebBase)
   {
      If(Test-Path -Path "$Item")
      {
         ...
         If($Log.IsPresent)
         {
            echo "`n[PATH] '${Item}'" >> "$pwd\Invoke-VaultCmd.report"
            Get-Content -Path "$Item" >> "$pwd\Invoke-VaultCmd.report"
         }
      }
   }
...
write-host "`n  MANUAL ACCESS CREDENTIAL MANAGER" -ForegroundColor DarkGreen
