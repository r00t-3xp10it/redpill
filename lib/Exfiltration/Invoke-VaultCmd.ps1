...
ForEach($Token in $Schemas)
{
   ...
   ForEach($Item in $Names)
   {
      ...
      $VaultEntrys.Rows.Add("DPAPI","$Token","$Item")|Out-Null
   }
}
...
If($Action -iMatch '^(check)$')
{
   ...
   $VaultEntrys | Format-Table -AutoSize | Out-String -Stream | Select-Object -SkipLast 2 | ForEach-Object {
...
If($Action -iMatch '^(delete)$')
{
   ...
   Else
   {
      ...
      $Results | Out-String -Stream | Select-Object -Skip 1|  Select-Object -SkipLast 1 | ForEach-Object {
         $stringformat = If($_ -Match '^(UserName)')
         {
            @{ 'ForegroundColor' = 'Green' }
         }
