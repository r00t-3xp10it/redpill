...
If($Action -iMatch '^(dump)$')
{
   ...
   If($Secure.IsPresent)
   {
      ...
      ForEach($Token in $CheckStoredCreds)
      {
         ...
         write-host "[" -NoNewline
         write-host "Resource" -ForegroundColor Green -NoNewline
         write-host "]::" -NoNewline
         write-host "$NOExtension"
         write-host "[" -NoNewline
         write-host "CredPath" -ForegroundColor Green -NoNewline
         write-host "]::" -NoNewline
         write-host "$Env:LOCALAPPDATA\$Token" -ForegroundColor DarkGray
         write-host "[" -NoNewline
         write-host "PSsecret" -ForegroundColor Green -NoNewline
