...
<#
.SYNOPSIS
   Generates Special-Character-Only encoded payload for a PowerShell command or script. Optionally it adds command
   line output to final command. All credit for this encoding technique goes to 牟田口大介 (@mutaguchi) who blogged
   about it in 2010: http://perl-users.jp/articles/advent-calendar/2010/sym/11
...
.DESCRIPTION
   Out-EncodedSpecialCharOnlyCommand encodes an input PowerShell scriptblock or path as a Special-Character-Only
   payload. The purpose is to highlight to the Blue Team that there are more novel ways to encode a PowerShell
   command other than the most common Base64 approach.
...
.PARAMETER Command
   Outputs the option to execute the specified commands (and any parameters) as
   though they were typed at the Windows PowerShell command prompt.
...
.PARAMETER PassThru
   (Optional) Avoids applying final command line syntax if you want to apply more obfuscation functions
   (or a different launcher function) to the final output.
...
.NOTES
   All credit for this encoding technique goes to 牟田口大介 (@mutaguchi) who blogged about it in 2010:
   http://perl-users.jp/articles/advent-calendar/2010/sym/11 This is a personal project developed by Daniel
   Bohannon while an employee at MANDIANT, A FireEye Company.
