...
.OUTPUTS
   * Powershell ROT13 chiper cmdlet.

   OriginalStringLines  : 1
   ConvertedStringChars : 6
   PlainTextString      : whoami
   ObfuscatedString     : jubnzv
...
write-host "* Powershell ROT13 chiper cmdlet." -ForegroundColor Green
...
try{
   ...
   If($Output -iMatch '^(log|logfile)$')
   {
      ...
      Write-Host "$OutPath\$RandomMe.log'`n" -ForegroundColor Green -NoNewline;
...
")
      ...
      Write-Host "*" -ForegroundColor Green -NoNewline;
      Write-Host " written to: '" -ForegroundColor DarkGray -NoNewline;
      Write-Host "$OutPath\$RandomMe.ps1" -ForegroundColor Green -NoNewline;
      Write-Host "'`n" -ForegroundColor DarkGray;
...
write-host "OriginalStringLines  : $Countlines" -ForegroundColor white
...
write-host "ObfuscatedString     : " -ForegroundColor white -NoNewline;
