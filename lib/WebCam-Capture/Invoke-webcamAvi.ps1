...
If($CheckInstall -iNotMatch '^(Python+\s+\d*)')
{
   ...
   If($ForceInstall.IsPresent)
   {
      ...
      write-host "Trying to silent install python3.`n" -ForegroundColor DarkYellow
   ...
   Else
   {
      ...
      write-host "  * Invoke -forceinstall for silent install.`n" -ForegroundColor DarkYellow
      return
   }
}
...
If(-not($OPenCVTest) -or ($OPenCVTest -iNotMatch 'True'))
{
   ...
   If($ForceInstall.IsPresent)
   {
      ...
      echo y|pip install opencv-python --exists-action ignore
      write-host ""
   }
   Else
   {
...
If($RecTime -gt 120 -or $RecTime -lt 15)
{
   [int]$RecTime = '15'
   write-host "  x " -ForegroundColor Red -NoNewline
