...
For($i=0; $i -lt $LoopRange; $i++)
{
   ...
   Start "https://www.google.com/search?q=Do+a+barrel+roll&sourceid=opera&ie=UTF-8&oe=UTF-8" -WindowStyle Maximized
   Start-Sleep -Seconds $LoopDelay
}

## Exit Execution Message Box
Powershell (New-Object -ComObject Wscript.Shell).PopUp("ACCESSO BLOCKEADO AO FORMANDO '$Env:COMPUTERNAME' RELACIONADO COM`nPESQUISAS PORNOGRAFIACAS REALIZADAS DURANTE O EXPEDIENTE.`n`n             - 'O SEU FORMADOR JA FOI AVISADO DO SUCEDIDO' -.",$MsgBoxClose,"                * Instituto do Emprego e Formação Profissional *",0+0)
