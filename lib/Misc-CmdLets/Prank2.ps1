...
.DESCRIPTION
   This module opens a new browser tab (default web Browser) and
   executes 'Do A Barrel Roll' google easter egg in predefined
   time loops. The CmdLet auto-delete itself in the end of exec
...
.Parameter OpenUrl
   The Url to open (default: google barrel roll)
...
.EXAMPLE
   PS C:\> powershell -WindowStyle Hidden -File Prank2.ps1 -LoopRange '3' -OpenUrl 'https://www.youtube.com/watch?v=XskZFjsguWE'
   Execute the cmdlet in a hidden windows and execute the prank 3 times (https://www.youtube.com/watch?v=XskZFjsguWE)
...
.OUTPUTS
   ...
   Loop Delay   : 30
   MsgBoxClose  : 25
   OpenUrl      : https://www.youtube.com/watch?v=XskZFjsguWE
...
[CmdletBinding(PositionalBinding=$false)] param(
   [string]$OpenUrl="https://www.google.com/search?q=do+a+barrel+roll",
   [string]$AutoDel="ON",     # AutoDelete the cmdlet after execution?
   [int]$MsgBoxClose='25',    # AutoClose the msgbox after 25 seconds.
...
write-host "MsgBoxClose  : $MsgBoxClose"
write-host "OpenUrl      : $OpenUrl`n"
...
For($i=0; $i -lt $LoopRange; $i++)
{
   ...
   # Exorcist scream: https://www.youtube.com/watch?v=XskZFjsguWE
   Start "$OpenUrl" -WindowStyle Maximized
