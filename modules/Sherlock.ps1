...
## Var declarations
$CveDataBaseId = "26"        ## 25 CVE's entrys available ($dATAbASE)
$CmdletVersion = "v1.3.9"    ## Sherlock CmdLet develop version number
$CVEdataBase = "07/01/2022"  ## Global $dATAbASE (CVE) last update date
...
function Get-Unquoted {
   ...
   $ColorTable = (gwmi -class Win32_Service -Property Name,DisplayName,PathName,StartMode,StartName | Where-Object {
      ...
      $VulnerableServiceName = $_.Name.ToString()
   }).PathName

   If($ColorTable)
   { 
      Write-Host "VulnId      : ::ACL (Mitre T1574)"
      Write-Host "ServiceName : $VulnerableServiceName"
      Write-Host "FolderPath  : $ColorTable" -ForegroundColor Green -BackgroundColor Black
