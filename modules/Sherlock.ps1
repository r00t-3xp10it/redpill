...
.EXAMPLE
   ...
   Get-Paths 1บ @arg accepts Everyone:(FileSystemRigths) value.

.EXAMPLE
   PS C:\> Get-Paths FullControl BUILTIN\Users
   SYNTAX: Get-Paths <FileSystemRigths> <IdentityReference>
   Get-Paths 2บ @arg accepts the Group Name (Everyone|BUILTIN\Users)
...
## Var declarations
$CveDataBaseId = "28"        ## 28 CVE's entrys available ($dATAbASE)
$CmdletVersion = "v1.3.10"   ## Sherlock CmdLet develop version number
$CVEdataBase = "17/01/2022"  ## Global $dATAbASE (CVE) last update date
...
$UserImput = $args[0] ## User Imputs
   ...
   $FindGroup = whoami /groups|findstr /C:"Everyone" /C:"Todos" /C:"Tout" /C:"Alle" /C:"Yัะต" /C:"Semua" /C:"Allemaal" /C:"Toate" /C:"Bce" /C:"Svi"|Select-Object -First 1
...
function Get-Unquoted {
   ...
   $FindEopBat = $args[0] ## FidEop.bat settings
   If(-not($FindEopBat) -or $FindEopBat -ieq $null){$FindEopBat = "Table"}
   If($FindEopBat -ieq "Table")
   {
      ## Create Data Table for output
      $MajorVersion = [int]$OSVersion.split(".")[0]
      $mytable = New-Object System.Data.DataTable
   ...
   $ColorTable = (gwmi -class Win32_Service -Property Name,DisplayName,PathName,StartMode,StartName | Where-Object {
   ...
   {
      $CountThisShit = 0
      ForEach($TokenPath in $ColorTable)
      {
         $CountThisShit++
         Write-Host "VulnId            : $CountThisShit::ACL (Mitre T1574)"
         Write-Host "FolderPath        : $TokenPath" -ForegroundColor Green -BackgroundColor Black
         Write-Host "StartName         : LocalSystem" -ForegroundColor Yellow
         Write-Host "StartMode         : Auto`n"
      }
   }
...
[int]$Count = 0 ## Loop counter
   ...
   .EXAMPLE
      PS C:\> Get-Paths Modify
      SYNTAX: Get-Paths <FileSystemRigths>
      Get-Paths arg accepts Everyone:(FileSystemRigths) value.

   .EXAMPLE
      PS C:\> Get-Paths FullControl BUILTIN\Users
      Get-Paths arg accepts the Group Name (Everyone|BUILTIN\Users)
...
[int]$Count = 0 ## Loop counter
   ...
   ForEach($Token in $ParseData){## Loop truth $ParseData services database List
      ...
      $CleanOutput = (Get-Acl -Path "$Token").Access|Where {## Search for Everyone:(F) registry service permissions (default)
         $_.IdentityReference -Match "$UserGroup" -and $_.RegistryRights -Match 'FullControl'
      }
      If($CleanOutput){$Count++ ##  Write the Table 'IF' found any vulnerable permissions
...
[int]$Count = 0 ## Loop counter
   ...
   If($MajorVersion -eq "vista"){
      ...
      $KBDataEntrys = "9"         ## Credits: @r00t-3xp10it (fully patch)
      $KB_dataBase = "26/12/2021" ## KB entrys database last update date
   }
   ...
   If($MajorVersion -eq 10){## Windows 10
      If($CPUArchitecture -eq "64 bits" -or $ProcessArchitecture -eq "AMD64"){
         $dATAbASE = @(## Windows 10 x64 bits
            "KB5007289","KB4562830","KB4577586","KB4580325",
            "KB5000736","KB5008212","KB5006753","KB5007273","KB5005699"
         )
...
function New-ExploitTable {
    ...
    $Global:ExploitTable.Rows.Add("PrintNigthmare Local Elevation of Privileges","N/A","CVE-2021-1675","https://github.com/calebstewart/CVE-2021-1675")
    $Global:ExploitTable.Rows.Add("HiveNigthmare Local Elevation of Privileges","N/A","CVE-2021-36934","https://github.com/cube0x0/CVE-2021-36934")
    $Global:ExploitTable.Rows.Add("Windows Installer Elevation of Privilege Vulnerability","N/A","CVE-2021-41379","https://github.com/klinix5/InstallerFileTakeOver")
}
...
function Find-CVE20211675 {

   <#
   .SYNOPSIS
      Author: r00t-3xp10it
      CVE-2021-1675 - PrintNigthmare Remote Escalation of Privileges
   ...
   $CVEID = "CVE-2021-1675"
   $ErrorActionPreference = "SilentlyContinue"
   $FilePath = $Env:WINDIR + "\System32\localspl.dll"

   $SoftwareVersion = (Get-Item "$FilePath" -EA SilentlyContinue).VersionInfo.ProductVersion
   If(-not($SoftwareVersion))
   {
      #localspl.dll not found
      $VulnStatus = "Not Vulnerable - localspl.dll missing"
   }
   Else
   {
      ...
      If($Major -lt 17763)
      {
         #localspl.dll vulnerable version found
         #CVE-2021-1675 Vulnerable registry keys
         $SpoolerServiceState = Get-ItemPropertyValue -Path "HKLM:\system\currentcontrolset\services\spooler" -Name start -ErrorAction SilentlyContinue
         $SpoolerRemoteRpc = Get-ItemPropertyValue -Path 'HKLM:\software\Policies\Microsoft\Windows NT\Printers' -Name RegisterSpoolerRemoteRpcEndPoint -ErrorAction SilentlyContinue
         $SpoolerPointAndPrint = Get-ItemPropertyValue -Path 'HKLM:\software\Policies\Microsoft\Windows NT\Printers' -Name PointAndPrint -ErrorAction SilentlyContinue

         #CVE-2021-1675 registry checks 
         If($SpoolerServiceState -ne $null -and $SpoolerServiceState -ne 4) #4 = Spooler Service disabled
         {
            If($SpoolerRemoteRpc -ne $null -and $SpoolerRemoteRpc -ne 2) #2 = RemoteRpc disabled
            {
               If($SpoolerPointAndPrint -ne $null -and $SpoolerPointAndPrint -ne 0) #0 = PointAndPrint disabled
               {
                  #ALL VULNERABLE SETTINGS ARE PRESENT
                  $VulnStatus = "Appears Vulnerable"
               }
               Else
               {
                  $VulnStatus = "Not Vulnerable - [remark] localspl.dll vulnerable version"     
               }
            }
            Else
            {
...
function Find-CVE202136934 {
   ...
   {
      $VulnStatus = "Not susceptible to CVE-2021-36934"
   }
   Else
   {
      #$TestAcl = icacls "$Env:WINDIR\system32\config\SAM"
      $TestAcl = (Get-Acl "C:\Windows\System32\config\SAM" -EA SilentlyContinue|Select-Object *).AccessToString
      ## Get Group Name (BUILTIN\users) in diferent languages
      # NOTE: England, Portugal, France, Germany, Indonesia, Holland, Romania, Croacia 
      $FindGroupUser = whoami /groups|findstr /C:"BUILTIN\Users" /C:"BUILTIN\Utilizadores" /C:"BUILTIN\Utilisateurs" /C:"BUILTIN\Benutzer" /C:"BUILTIN\Pengguna" /C:"BUILTIN\Gebruikers" /C:"BUILTIN\Utilizatori" /C:"BUILTIN\Korisnici"|Select-Object -First 1
      $SplitStringUser = $FindGroupUser -split(" ");$RawNameUsers = $SplitStringUser[0] -replace ' ','';$GroupNameUsers = $RawNameUsers -replace '\\','\\'

      If($TestAcl -iMatch '(Failed processing 1 files)$')
      {
         $VulnStatus = "Not Vulnerable"
      }
      Else
      {
         if($TestAcl -iMatch "($GroupNameUsers)" -and $TestAcl -iMatch '(ReadAndExecute)$')
         #If($TestAcl -iMatch "${GroupNameUsers}:\(I\)\(RX\)$")
         {
            $VulnStatus = "Appears Vulnerable"
         }
         Else
...
function Find-CVE202141379 {

   <#
   .SYNOPSIS
      Author: r00t-3xp10it
      CVE-2021-41379 - Windows installer elevation of privileges
   ...
   .NOTES
      For the exploit to work, an attacker must already have access to
      the targeted Windows machine and Microsoft Edge must be installed on it
   #>
   ...
   #Debug: '55c92734-d682-4d71-983e-d6ec3f16059f' - 'test pkg'
   #Detect previous exploitation of this PoC by monitoring for EventID 1033 and "test pkg"
   $CheckExploit = Get-WinEvent -LogName "application" -EA SilentlyContinue | Where-Object {
      $_.Id -eq 1033 -and $_.Message -Match 'test pkg'
   }| Select-Object -Property TimeCreated

   If($CheckExploit)
   {
      $VulnStatus = "Vulnerable - Payload [test pkg] found"
   }
   Else
   {
      $VulnStatus = "Not Vulnerable? - [test pkg] not found"   
   }
   Set-ExploitTable $CVEID $VulnStatus
}