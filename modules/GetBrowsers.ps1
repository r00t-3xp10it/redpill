...
function BROWSER_RECON {
    ...
    {
       $OPfound = "Found"
       If(Test-Path -Path "$Env:LOCALAPPDATA\Programs\Opera GX\launcher.exe" -EA SilentlyContinue)
       {
          $OPData = (Get-ChildItem -Path "$Env:LOCALAPPDATA\Programs\Opera GX\launcher.exe").VersionInfo.ProductVersion.ToString()
       }
       Else{$OPData = "{null}       "}
    }
    Else
    ...
    If(-not($MInvocation) -or $MInvocation -eq $null){$id = "{Null}";$fd = "{Null}";$cd = "{Null}";$OP = "{Null}"}Else{}
    ...
    echo "OPERA GX  $OPfound  $OStatus   $OPData   $OP" >> $LogFilePath\BrowserEnum.log
...
Get-Content "$LogFilePath\BrowserEnum.log" | Format-Table -AutoSize | Out-String -Stream | ForEach-Object {
   $stringformat = If($_ -iMatch '^(Browser|Status)' -or $_ -iMatch '(True)$'){
