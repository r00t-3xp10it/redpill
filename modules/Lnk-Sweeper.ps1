...
$host.UI.RawUI.WindowTitle = "@Lnk-Sweeper v3.3.11 {SSA@RedTeam}"
...
If($TimeStamp -ieq "Today" -or $TimeStamp -Match '^(\d+|\d+\d+)/(\d+|\d+\d+)/(\d+\d+\d+\d+)$')
{
   ...
   If($Paranoid -ieq "false"){$LocationsToScanList = "$Directory"}
   ForEach($Token in $LocationsToScanList)
   {
      ...
      .NOTES
         ...
         It will make the cmdlet to scan a list of pre-defined directorys
         insted of scanning user input directory or the default one.
