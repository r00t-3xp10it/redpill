...
$host.UI.RawUI.WindowTitle = "@Lnk-Sweeper v3.4.15 {SSA@RedTeam}"
...
ElseIf($TimeStamp -ieq "Year" -or $TimeStamp -Match '^(\d+\d+\d+\d+)$')
{
   ...
   If($Bypass -ieq "false")
   {
      ...
      If($Action -ieq "clean" -and $LocationsToScanList -iMatch "^(C:\\Users\\$Env:USERNAME|C:\\Windows)$")
      {
         Write-Host "[x] Warning : 'Delete' lnk files in 'SYSTEM' directorys with 'Year' in timestamp!" -ForegroundColor Red -BackgroundColor Black
         Start-Sleep -Seconds 1;Write-Host "    Bypass  : Invoke cmdlet with -bypass 'true' parameter to bypass restrictions."
         Write-Host "[i] Aborting: Unload Lnk-Sweeper module from `$PSHOME .." -ForegroundColor Yellow
