...
$host.UI.RawUI.WindowTitle = "@Lnk-Sweeper v3.4.14 {SSA@RedTeam}"
...
ElseIf($TimeStamp -ieq "Year" -or $TimeStamp -Match '^(\d+\d+\d+\d+)$')
{
   ...
   {
      Write-Host "[x] Warning  : Delete lnk files only with 'Year' in timestamp detected!" -ForegroundColor Red -BackgroundColor Black
      Write-Host "    Action   : Pausing execution for 10 sec, Proceed at your own risk .." -ForegroundColor Yellow
      [string]$OutLog = "true" #Record (in logfile) the next steps taken just in case ;)
