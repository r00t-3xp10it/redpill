...
ElseIf($TimeStamp -ieq "Year" -or $TimeStamp -Match '^(\d+\d+\d+\d+)$')
{
   ...
   {
      Write-Host "[x] Warning  : Messing with 'SYSTEM' directory(s) Recursive!" -ForegroundColor Red -BackgroundColor Black
      Write-Host "    Remark   : Giving 10 sec time, to manualy exit cmdlet execution .." -ForegroundColor Yellow
