...
ElseIf($TimeStamp -ieq "Year" -or $TimeStamp -Match '^(\d+\d+\d+\d+)$')
{
   ...
   {
      Write-Host "[x] Warning  : Messing with 'SYSTEM' directorys recursive detected!" -ForegroundColor Red -BackgroundColor Black
      Write-Host "    Action   : Pausing for 10 seconds, to manualy exit cmdlet execution?" -ForegroundColor Yellow
      [string]$OutLog = "true" #Record (in logfile) the steps taken just in case ;) ...
