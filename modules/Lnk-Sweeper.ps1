...
ElseIf($TimeStamp -ieq "Year" -or $TimeStamp -Match '^(\d+\d+\d+\d+)$')
{
   ...
   {
      Write-Host "[i] Warning  : Messing with Dangerouse directory Recursive!" -ForegroundColor Red -BackgroundColor Black
      Write-Host "    Warning  : Giving 10 seconds of time, for abortion if decided." -ForegroundColor Yellow
