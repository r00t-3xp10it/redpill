...
ElseIf($TimeStamp -ieq "Year" -or $TimeStamp -Match '^(\d+\d+\d+\d+)$')
{
   ...
   If($Action -ieq "clean" -and $Directory -iMatch "^(C:\\Users\\$Env:USERNAME|C:\\Windows)$")
   {
      Write-Host "[i] Warning  : Messing with '$Directory' structure recursive!" -ForegroundColor Red -BackgroundColor Black
      Write-Host "    Warning  : Giving 8 seconds of time, for abortion if decided." -ForegroundColor Yellow
