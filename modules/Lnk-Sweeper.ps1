...
.EXAMPLE
   PS C:\> .\Lnk-Sweeper.ps1 -action clean -Directory "$Env:USERPROFILE\Desktop" -TimeStamp Year
...
.INPUTS
   None. You cannot pipe objects into Lnk-Sweeper.ps1
...
$host.UI.RawUI.WindowTitle = "@Lnk-Sweeper v3.4.18 {SSA@RedTeam}"
...
If($TimeStamp -ieq "Today" -or $TimeStamp -Match '^(\d+|\d+\d+)/(\d+|\d+\d+)/(\d+\d+\d+\d+)$')
{
   ...
   ForEach($Token in $LocationsToScanList)
   {
      ...
      If($OutLog -ieq "true")
      {
         Write-Host "              LogFile  : $MyStorage\Lnk-Sweeper.txt" -ForegroundColor Yellow
      }
      Write-Host "              ------------------------------------------------------------------"

      Start-Sleep -Milliseconds 400
      #-and $_.FullName -iNotMatch "($ForbiddenLocation)"
      ...
      Get-ChildItem -Path "$Token" -Include *.lnk -Recurse | Select-Object * | Where-Object { 
         ...
         $_.CreationTime.Year -Match "$MyYear" -and $_.FullName -iNotMatch "$Env:USERNAME\\Links" } | ForEach-Object {
...
ElseIf($TimeStamp -ieq "Year" -or $TimeStamp -Match '^(\d+\d+\d+\d+)$')
{
   ...
   ForEach($Token in $LocationsToScanList)
   {
      ...
      If($Bypass -ieq "false")
      {
         ...
         .SYNOPSIS
            Author: @r00t-3xp10it
            Helper - Bypass 'Year' timestamp deletion restriction!

         .EXAMPLE
            PS C:\> .\Lnk-Sweeper.ps1 -Bypass "true"
            Bypass 'Year' timestamp deletion restriction!
         #>

         If($Action -ieq "clean" -and $LocationsToScanList -iMatch "^(C:\\Users\\$Env:USERNAME|C:\\Users\\$Env:USERNAME\\Links|C:\\Windows)$")
         {
            Write-Host "[x] Forbiden: Recurse delete lnk files from '$Token' only with 'YEAR' in timestamp!" -ForegroundColor Red -BackgroundColor Black
            Start-Sleep -Seconds 1;Write-Host "    Help    : Invoke cmdlet again with -bypass 'true' argument to bypass restriction .." -ForegroundColor Yellow
...
Else
{
   ...
   Write-Host "----------------------------------------------------------"
   Write-Host "    Syntax  : .\Lnk-Sweeper.ps1 -TimeStamp `"year`""
   Write-Host "    Syntax  : .\Lnk-Sweeper.ps1 -TimeStamp `"${MyYear}`""
   Write-Host "    Syntax  : .\Lnk-Sweeper.ps1 -TimeStamp `"today`""
