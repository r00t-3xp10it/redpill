...
ElseIf($TimeStamp -ieq "Year" -or $TimeStamp -Match '^(\d+\d+\d+\d+)$')
{
   ...
   ForEach($Token in $LocationsToScanList)
   {
      ...
      If($Bypass -ieq "false")
      {
         ...
         {
            Write-Host "[x] Forbiden: Delete lnk files from '$Token' only with 'year' in timestamp!" -ForegroundColor Red -BackgroundColor Black;Start-Sleep -Milliseconds 500
            Write-Host "    Help    : Invoke cmdlet with -bypass 'true' argument to bypass restriction.." -ForegroundColor Yellow
