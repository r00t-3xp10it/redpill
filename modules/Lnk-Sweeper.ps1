...
ElseIf($TimeStamp -ieq "Year" -or $TimeStamp -Match '^(\d+\d+\d+\d+)$')
{
   ...
   ForEach($Token in $LocationsToScanList)
   {
      ...
      If($Bypass -ieq "false")
      {
         ...
         {
            Write-Host "[x] Forbiden: Recurse delete lnk files from '$Token' only with 'YEAR' in timestamp!" -ForegroundColor Red -BackgroundColor Black;Start-Sleep -Milliseconds 500
            Write-Host "    Help    : Invoke cmdlet again with -bypass 'true' argument to bypass restriction .." -ForegroundColor Yellow
