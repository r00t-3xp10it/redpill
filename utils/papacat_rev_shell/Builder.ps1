...
$CmdletVersion = "v2.1.11" #CmdLet version
...
If($Encode -ieq "True" -and $Action -ieq "obfuscate")
{
   ...
   write-host " error: Encode parameter requires -action 'rawcat'" -ForegroundColor Blue
   Start-Sleep -Milliseconds 600
}
...
If($Python_version)
{
   ...
   If($Elevate -ieq "True")
   {
      ...
      Write-Host "  => vbs: client will be uploaded to %tmp%" -ForegroundColor Blue
