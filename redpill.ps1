...
If($EOP -ieq "Verbose" -or $EOP -ieq "Enum"){
   ...
   $SizeDump = ((Get-Item -Path "$Env:TMP\sherlock.ps1" -EA SilentlyContinue).length/1KB) ## Default => 84,6484375/KB
...
If($PingSweep -ieq "enum" -or $PingSweep -ieq "portscan")
{
   ...
   #Run auxiliary module
   powershell -File "$Env:TMP\PingSweep.ps1" -Action $PingSweep -IpRange "$IpRange" -ScanType $ScanType -output $output -LogFile $LogFile
...
$HelpParameters = @"
   ...
   .DESCRIPTION
      CmdLet to enumerate active IP address on Local Lan for possible Lateral
      movement oportunitys. It reports active Ip address, trys to resolve dns
      hostname and scans for possible open ports if invoked -PingSweep 'portscan'.
   ...
   .NOTES
      ...
      but it will be substracted from further tests\scans\logs\report_tables.
      Remark: PingSweep module migth take a long time to finish depending of
      -IpRange 'int' argument setting and if -PingSweep 'PortScan' its invoked.
   ...
   .OUTPUTS
      * Scanning local lan for active devices!
      ...
      Name  Status LinkSpeed Action   IpRange LogFile
      ----  ------ --------- ------   ------- -------
      Wi-Fi Up     65 Mbps   portscan 64,73   true
