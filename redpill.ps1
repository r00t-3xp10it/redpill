...
If($PingSweep -ieq "Enum" -or $PingSweep -ieq "Verbose"){
   ...
   If(-not(Test-Path -Path "$Env:TMP\PingSweep.ps1")){## Download PingSweep.ps1 from my GitHub repository
      ...
      If($SizeDump -lt 9){## Corrupted download detected => DefaultFileSize: 9,458984375/KB
         Write-Host "[error] Abort, Corrupted download detected" -ForegroundColor Red -BackgroundColor Black
...
If($CleanTracks -ieq "Clear" -or $CleanTracks -ieq "Paranoid"){
   ...
   If(-not(Test-Path -Path "$Env:TMP\CleanTracks.ps1")){## Download CleanTracks.ps1 from my GitHub repository
      ...
      If($SizeDump -lt 11){## Corrupted download detected => DefaultFileSize: 11,1982421875/KB
         Write-Host "[error] Abort, Corrupted download detected" -ForegroundColor Red -BackgroundColor Black
...
$HelpParameters = @"
   ...
   .NOTES
      ...
      'Paranoid' deletes all eventvwr logs if exec with admin privs

   .Parameter CleanTracks
      Accepts arguments: Clear and Paranoid (default: Clear)

   .Parameter Verb
      Print eventvwr logs beeing cleared (default: false)
   ...
   .EXAMPLE
      ...
      Clean artifacts in paranoid mode and print eventvwr logs cleared

   .OUTPUTS
      * Cover activity tracks on SKYNET
        => Token: Administrator privileges.
        => Clear: Multiple locations\Regedit.
        => Clear: StartUp directory artifacts.
        => Clear: ALL LogFiles from Eventvwr snapin.
           Clean: Microsoft-Windows-Windows Defender/WHC
           Clean: Microsoft-Windows-AppModel-Runtime/Admin
           Clean: Microsoft-Windows-Websocket-Protocol-Component/Tracing

      Module      Date     ItemsDeleted ModifiedRegKeys ScriptsDeleted
      ----------- ----     ------------ --------------- --------------
      CleanTracks 22:17:29 1110         10              3
   #>!bye..
