...
If($GetPasswords -ieq "Enum" -or $GetPasswords -ieq "Dump"){
   ...
   If(-not(Test-Path -Path "$Env:TMP\GetPasswords.ps1")){## Download GetPasswords.ps1 from my GitHub repository
      ...
      If($SizeDump -lt 16){## Corrupted download detected => DefaultFileSize: 16,9609375/KB
         Write-Host "[error] Abort, Corrupted download detected" -ForegroundColor Red -BackgroundColor Black
...
if($AppLocker -ieq "Enum" -or $AppLocker -ieq "WhoAmi" -or $AppLocker -ieq "TestBat" -or $AppLocker -Match '\\'){
   ...
   If(-not(Test-Path -Path "$Env:TMP\AppLocker.ps1")){## Download AppLocker.ps1 from my GitHub repository
      ...
      If($SizeDump -lt 18){## Corrupted download detected => DefaultFileSize: 18,201171875/KB
         Write-Host "[error] Abort, Corrupted download detected" -ForegroundColor Red -BackgroundColor Black
...
If($UacMe -ne "false"){
   ...
   If(-not(Test-Path -Path "$Env:TMP\UacMe.ps1")){## Download UacMe.ps1 from my GitHub repository
      ...
      If($SizeDump -lt 22){## Corrupted download detected => DefaultFileSize: 22,4541015625/KB
         Write-Host "[error] Abort, Corrupted download detected" -ForegroundColor Red -BackgroundColor Black
