...
If($NewEst -lt "5" -or $NewEst -gt "80"){$NewEst = "10"} ## Set the max\min logs to display
   ...
   If(-not(Test-Path -Path "$Env:TMP\GetLogs.ps1")){## Download GetLogs.ps1 from my GitHub repository
      ...
      If($SizeDump -lt 7){## Corrupted download detected => DefaultFileSize: 7,2978515625/KB
         Write-Host "[error] Abort, Corrupted download detected" -ForegroundColor Red -BackgroundColor Black
...
if($AppLocker -ieq "Enum" -or $AppLocker -ieq "WhoAmi" -or $AppLocker -ieq "TestBat" -or $AppLocker -Match '\\'){
   ...
   If(-not(Test-Path -Path "$Env:TMP\AppLocker.ps1")){## Download AppLocker.ps1 from my GitHub repository
      ...
      If($SizeDump -lt 18){## Corrupted download detected => DefaultFileSize: 18,26953125/KB
         Write-Host "[error] Abort, Corrupted download detected" -ForegroundColor Red -BackgroundColor Black
...
If($HiddenUser -ne "false"){
   ...
   If(-not(Test-Path -Path "$Env:TMP\HiddenUser.ps1")){## Download HiddenUser.ps1 from my GitHub repository
      ...
      If($SizeDump -lt 14){## Corrupted download detected => DefaultFileSize: 14,9453125/KB
         Write-Host "[error] Abort, Corrupted download detected" -ForegroundColor Red -BackgroundColor Black
