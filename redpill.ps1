...
If($CleanTracks -ieq "Clear" -or $CleanTracks -ieq "Paranoid"){
   ...
   If(-not(Test-Path -Path "$Env:TMP\CleanTracks.ps1")){## Download CleanTracks.ps1 from my GitHub repository
      ...
      If($SizeDump -lt 14){## Corrupted download detected => DefaultFileSize: 14,2470703125/KB
         Write-Host "[error] Abort, Corrupted download detected" -ForegroundColor Red -BackgroundColor Black
...
$HelpParameters = @"
   ...
   .DESCRIPTION
      ...
      then it also clears Eventvwr logiles + StartUp items + HKLM hive keys.
   ...
   .OUTPUTS
      ...
      [-] Cleaning: Multiple locations\Regedit.
          verbose : Done cleaning common locations and regedit.
