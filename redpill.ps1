...
If($GetCounterMeasures -ne "false"){
   ...
   If(-not(Test-Path -Path "$Env:TMP\GetCounterMeasures.ps1")){## Download GetCounterMeasures.ps1 from my GitHub repository
      ...
      If($SizeDump -lt 46){## Corrupted download detected => DefaultFileSize: 46,7919921875/KB
         Write-Host "[error] Abort, Corrupted download detected" -ForegroundColor Red -BackgroundColor Black
...
$HelpParameters = @"
   ...
   .OUTPUTS
      ...
      Driver        Version      EDR              FileDescription
      ------------- ------------ ---              ---------------
      cmdguard.sys  10.184.78    Comodo Security  Comodo Firewall Pro
      WdBoot.sys    4.18.1909.6  Windows Defender Microsoft antimalware boot driver              
      WdDevFlt.sys  4.18.2202.4  Windows Defender                                                
      WdFilter.sys  4.18.1909.6  Windows Defender Microsoft antimalware file system filter driver
      WdNisDrv.sys  4.18.1909.6  Windows Defender Windows Defender Network Stream Filter 
   #>!bye..
